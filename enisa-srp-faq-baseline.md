...
retrieved: 2026-09-14 (fetched 03:15 UTC, unchanged since 23:15 UTC on 2026-09-13)
guidance_retrieved: 2026-09-14 (fetched 03:15 UTC, three of the four guidance subpages — AR User Registration, AR Notification Submission and Update, AR Interface Functions — were restructured from arrow-chained prose into headed Pre-conditions/Steps/Expected-result blocks; PEC untouched — see change log)
glossary_retrieved: 2026-09-14 (fetched 03:15 UTC, unchanged since 23:15 UTC on 2026-09-13 — see `enisa-srp-glossary-baseline.md`)
csirt_list_retrieved: 2026-09-14 (fetched 03:15 UTC, unchanged since 17:11 UTC on 2026-09-12)
ar_user_manual_retrieved: 2026-09-14 (fetched 03:15 UTC, wording unchanged since first capture at 10:12 UTC on 2026-09-10)
terms_conditions_retrieved: 2026-09-14 (fetched 03:15 UTC, unchanged since the trailing space was lost at 14:07 UTC on 2026-09-10)
ar_user_tutorial_video_retrieved: 2026-09-14 (fetched 03:15 UTC, unchanged since first capture at 11:10 UTC on 2026-09-11)
...
- Early Warning: the notification is stored and is accessible to the CSIRT Designated as Coordinator (CDaC) and to ENISA. An email and an alert will be sent to this CSIRT, and also to you and all other ARs of the selected manufacturer, confirming submission, and automatically to ENISA. The state of a successfully submitted notification is updated to "Early Warning". Concerned CSIRTs will receive the Early Warning only after manual dissemination by the CSIRT Designated as Coordinator.
- 72-hour Notification: the state of a successfully submitted notification is updated to "72h Submitted". The dissemination state of a notification subject to Particularly **Exception** Circumstances (PEC) *(typo still present — "Exceptional" missing its "-al", reproduced verbatim; occurs three times in this section, same as before)* is updated to `"72h Submitted under PEC "` *(new stray space before the closing quote mark since 2026-09-14, not previously present — reproduced verbatim)*. An email and an alert will be sent to the CDaC, and also to you and all other ARs of the selected manufacturer, confirming submission, and to ENISA — but ENISA automatically receives the 72h Notification only when Particularly Exception Circumstances have not been invoked. Concerned CSIRTs will receive the 72-hour Notification only after manual dissemination by the CSIRT Designated as Coordinator.
...
Purpose: as Primary AR, invite a Secondary AR for the same manufacturer to register on the SRP. Pre-conditions:
...
A pop-up confirming the invitation has been sent is then displayed. Expected result: "The SRP sends an email invitation to the Secondary AR. A new user record is created in the SRP with the specified email address and corresponding manufacturer details. The user role is assigned once the registration is completed." *(this precondition/expected-result sentence lost its own bold wrapping too, since 2026-09-14; wording unchanged — still correctly says the invitation goes to the Secondary AR, and the "Pending Invitation" status name is still not mentioned.)*
...
*Search, Sort & Filter Notifications* — Purpose: Follow these steps to search, sort and filter the notifications accessible to you. The actions you can perform: searching for notifications; sorting notification records; and filtering results based on specific criteria. Preconditions: your user status is "Active"; you are logged in to the SRP; at least one notification has previously been submitted or saved as a draft. Steps: access the Dashboard; search by Notification ID, Manufacturer, or Title; sort by Title (alphabetical order) or Last Update (chronological order); filter by selecting "All filters", narrowing by Member States in which the product with digital elements is available and Type of submission; "You can select the appropriate tab to focus on notifications with a pending action label." (the "Action Required" tab is still not named by that label); select "Clear all filters" to remove all applied filters. Exception: "In case no results are found, the search returns no results, and the system informs you that no notifications match the search criteria." Expected result: "The results are filtered and/or sorted based on your selections."
