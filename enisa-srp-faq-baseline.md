...
FAQ Q9 was substantively rewritten to broaden who can see a manufacturer's notifications, regaining its "[UPDATED]" tag and moving the FAQ page's own stamp for the first time since 12 September; the rewrite now contradicts the AR Interface Functions guidance page, which is itself unchanged. All other ten pages match the 2026-09-17 09:00 UTC baseline verbatim.

**Changed**
Q9's access-permission paragraph was replaced. It read: "The Primary AR can access all notifications associated with the manufacturer, whereas a Secondary AR can access only the notifications they submitted themselves." It now reads, across two paragraphs: "Primary and Secondary ARs associated with the same manufacturer can access, view, and update notifications associated with that manufacturer, regardless of with AR originally submitted them. This allows another AR associated with the same manufacturer to continue the reporting process, including subsequent reporting stages and updates, as applicable. The only exception concerns draft notifications. Drafts are stored locally in the individual AR's account and are therefore not visible to other ARs associated with the same manufacturer." followed, unchanged in substance, by "A Secondary AR does not have the same administrative permissions as the Primary AR, but may claim the Primary AR role, subject to review and approval by the designated CSIRT." (which also gained a comma before "but"). So the rule flips from "a Secondary AR sees only what they submitted" to "any AR on the manufacturer sees everything except the other AR's drafts." The new sentence reproduces ENISA's own grammar slip verbatim — "regardless of with AR" rather than "regardless of which AR". Q9 regained its "[UPDATED]" tag (lost 2026-09-11 13:17 UTC), and the FAQ page-level stamp moved "Updated: 12 September 2026" → "Updated: 17 September 2026", correlating with this edit for the first time since the stamp itself proved unreliable.

**Watch**
The AR Interface Functions guidance page's "View the Dashboard" section is untouched and still reads: "A Primary AR can access all notifications associated with their manufacturer, whereas a Secondary AR can only access notifications they submitted and drafts they created. Secondary ARs cannot view notifications submitted by another AR associated with the same manufacturer." That is exactly the access model Q9 just abandoned — the two ENISA pages now describe two different, incompatible notification-visibility rules. Worth flagging to ENISA; not resolved by this baseline.

**Editorial**
One further stray nbsp turned up on Q7, not part of the 2026-09-17 pass ("...severe incident.&nbsp;", first paragraph) — one occurrence, reproduced in place below.

**Unchanged**
Main page (Content navigation still 9 entries, all cards and dates as logged); FAQ Q1–Q8, Q10–Q31, and the closing note byte for byte; CSIRT list (27 countries, same links, "Last updated: 10 September 2026"); all four guidance subpages otherwise byte for byte (same stamps — "12 September 2026" ×3, PEC "09 September 2026" — same tracked defects); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; Glossary (still v1.3, 39 fields, all rows and both footnotes re-verified) — see `enisa-srp-glossary-baseline.md`.

### 2026-09-17 09:00 UTC (vs. 2026-09-15 06:11 UTC)

A page-wide non-breaking-space insertion pass hit nine of the eleven tracked pages, alongside one real structural/punctuation change on the Terms and Conditions page and a small heading/step-text tweak on two guidance sections.

**Changed**
The Terms and Conditions page's single page-text paragraph split into two: the lead-in sentence now ends in a colon instead of running straight into the link, and the link itself lost its trailing full stop. Link target and text are unchanged.

**Editorial**
A page-wide non-breaking-space insertion pass hit roughly 50+ spots across 9 of 11 pages (main, FAQ, all four guidance pages, AR User Manual, Glossary) — mostly sentence-final, five mid-sentence on FAQ Q1 — plus a "Steps"→"Steps:" heading change on two guidance sections (AR Notification Submission and Update's Final Report, and AR Interface Functions' Delete AR–Manufacturer Association) and a step-text restructuring on AR Interface Functions' Delete Association step (one step split across two `<strong>` tags, its follow-on sentence moved out of the bullet into its own paragraph). Full detail in the sections below. CSIRT list and AR User Tutorial Video are untouched.

**Watch**
The Glossary's two footnotes each gained a run of 32 consecutive `&nbsp;` entities appended after their text — a markedly more extreme instance of this run's pattern than anywhere else on any of the eleven pages. Worth a closer look next check in case it signals a migration/export artifact rather than an ordinary edit.

**Unchanged**
CSIRT list (27 countries, same mappings) and AR User Tutorial Video byte for byte; Content navigation still 9 entries; Glossary still 39 fields, version footer still "Version 1.3. Last update: 10 September 2026".

### 2026-09-15 06:11 UTC (vs. 2026-09-14 19:10 UTC)

One purely typographic change found: AR Interface Functions' View Alerts precondition gained an invisible non-breaking space. No wording, deadline, obligation, or field changed on any of the other ten tracked pages.

**Editorial**
"you have a valid account with EU Login (for AR) and can authenticate successfully" → "...(for AR)**&nbsp;**and can authenticate successfully", on AR Interface Functions only — the same single-nbsp-insertion pattern already tracked on this page's Add Additional Manufacturer Association step (2026-09-14 18:13 UTC).

**Unchanged**
Main page (Content navigation still 9 entries), FAQ (31 Q&As), the other three guidance subpages (AR User Registration, AR Notification Submission and Update, PEC), AR User Manual, Terms and Conditions, AR User Tutorial Video, the Glossary (39 fields, footer "Version 1.3. Last update: 10 September 2026", unchanged since 2026-09-12), and the CSIRT list (27 countries, "Last updated: 10 September 2026") all match the 19:10 UTC baseline verbatim.

### 2026-09-14 19:10 UTC (vs. 2026-09-14 18:13 UTC)

One purely typographic change found: AR Notification Submission and Update's 72-hour Notification "Expected results" bullet capitalised "the state" to "The state" right after its own colon, breaking the parallelism with the still-lowercase Draft and Final Report bullets either side of it. No wording, deadline, obligation, or field changed on any of the other ten tracked pages.

**Editorial**
"72-hour Notification: the state of a successfully submitted notification..." → "72-hour Notification: **The** state of a successfully submitted notification...", on AR Notification Submission and Update only.

**Unchanged**
Main page (Content navigation still 9 entries), FAQ (31 Q&As), the other three guidance subpages (AR User Registration, AR Interface Functions, PEC), AR User Manual, Terms and Conditions, AR User Tutorial Video, the Glossary (39 fields, footer "Version 1.3. Last update: 10 September 2026", unchanged since 2026-09-12), and the CSIRT list (27 countries, "Last updated: 10 September 2026") all match the 18:13 UTC baseline verbatim.

### 2026-09-14 18:13 UTC (vs. 2026-09-14 17:06 UTC)

AR Interface Functions picked up four cosmetic wording edits to its Pre-conditions/Preconditions/Precondition sentences; no wording, deadline, obligation, or field changed on any of the other ten tracked pages.

**Editorial**
Four "your user status is..." precondition sentences on AR Interface Functions (View Personal and Update Manufacturer Details, Add Additional Manufacturer Association, the top-level Dashboard section, and View the Dashboard) had "your" capitalised to "Your"; Add Additional Manufacturer Association's Exceptions sentence also gained an invisible non-breaking space before "Manufacturer Name" ("such as Manufacturer Name" → "such as&nbsp;Manufacturer Name"). The page's own "Last updated: 12 September 2026" stamp did not move despite the edits, continuing the pattern that this stamp is not a reliable change signal.

**Unchanged**
Main page, FAQ (31 Q&As), the other three guidance subpages (AR User Registration, AR Notification Submission and Update, PEC), AR User Manual, Terms and Conditions, AR User Tutorial Video, the Glossary (39 fields, footer "Version 1.3. Last update: 10 September 2026", unchanged since 2026-09-12), and the CSIRT list (27 countries, "Last updated: 10 September 2026") all match the 17:06 UTC baseline verbatim.
...
AR Interface Functions completed its page-wide restructuring — its last four flowing-prose sections are now headed Steps blocks — and AR Notification Submission and Update split a closing dissemination sentence into two bullets; no wording, deadline, obligation, or field changed anywhere. All other pages are unchanged.

**Changed**

- **AR Interface Functions' last four sections restructured into headed Steps blocks.** "View the Dashboard", "Search, Sort & Filter Notifications", "View Notification Details", and "View Alerts" each move from a flowing "Steps: ..." sentence to a `<h4>Steps</h4>` heading followed by one bullet per step (Search/Sort/Filter's Steps also gains nested sub-bullets for its Search criteria, sort columns, and narrowing criteria) — completing the conversion already applied to the page's earlier sections. Same facts throughout: no criterion, column, filter, or status name changed. Two small side effects: "View Notification Details"'s step now reads "Click" with a capital C (was lowercase); "View Alerts"'s step now reads "Open the Alerts tab" with a capital O and no trailing full stop (was lowercase "open the Alerts tab.").
- **AR Notification Submission and Update's "Update an Existing Notification" closing sentence split into two bullets.** "The SRP automatically notifies the CDaC via an alert and an email, and automatically notifies ENISA and any concerned CSIRTs..." (one comma-joined sentence) is now two separate bullets, the subject repeated in each; the first bullet has no trailing full stop, the second keeps its own.

**Watch**

- The FAQ intro paragraph, never previously checked at Q1–Q31's character-level precision, now shows curly quotes/apostrophe and three `&nbsp;` artifacts not reproduced in this baseline's own quoted text. Provenance unclear — flagged for the record, not counted as a change.

**Editorial**

Three more trailing-full-stop drops on AR Notification Submission and Update's Steps bullets (the Update section's "Open a notification..." and "Click 'Update'..." bullets, and the Final Report Steps' "Submit the Final Report..." bullet — the latter isolated from its still-punctuated Early Warning/72-hour siblings); no wording changed.

**Unchanged**

Main page (Content navigation still 9 entries); FAQ Q1–Q31 ("Updated: 12 September 2026"); CSIRT list (27 countries, "Last updated: 10 September 2026"); Glossary (still v1.3, 39 fields, all rows re-verified) — see `enisa-srp-glossary-baseline.md`; AR User Registration and PEC guidance byte for byte; AR User Manual, Terms and Conditions, and AR User Tutorial Video byte for byte; both affected guidance pages' "Last updated" stamps still "12 September 2026", unmoved.

### 2026-09-14 09:14 UTC (vs. 2026-09-14 05:17 UTC)

One real content change — AR Interface Functions dropped a clause naming the "Action Required" tab — plus a sweeping straight→curly quote conversion and outer-quote removal across all four guidance pages that surfaced two new mismatched-quote defects and a doubled-style term; the FAQ page picked up more space-inside-bold shifts. Main page, CSIRT list, Glossary, AR User Manual, Terms and Conditions, and Tutorial Video are all unchanged.

**Changed**

- **AR Interface Functions' "Search, Sort & Filter Notifications" step dropped its parenthetical** — "(the "Action Required" tab is still not named by that label)", which had followed the "pending action label" sentence since the previous check, is simply gone. The tab's on-screen name is now unstated rather than explicitly flagged as absent — a different, smaller gap than before.

**Watch**

- **Two new mismatched quote pairs**: the "your user status is 'Active'" precondition on both AR Notification Submission and Update's "Update an Existing Notification" and AR Interface Functions' "Add Additional Manufacturer Association" now opens with a curly quote and closes with a straight one — reproduced verbatim on each page.
- **AR Interface Functions' Add-Manufacturer "Unverified" status is now quoted twice in one paragraph with two different styles** (curly, then straight further down) — both were straight before.

**Editorial**

Every "Exceptions:"/"Expected result:" sentence this baseline had reproduced wrapped in outer quotation marks, across all four guidance pages, now appears unquoted on the live page; most inline UI/status-name quotes on the same pages turned from straight to curly ("Continue", "Save", "Active", status names, etc.), a handful stayed straight. AR Notification Submission and Update's "Update" step-quote flipped to curly single (the opposite style from its own Exceptions sentence); its 72-hour/Final-Report preconditions and AR Interface Functions' Search/Sort/Filter preconditions became genuine bulleted lists, and every Early-Warning/72-hour/Final-Report step bullet on the Notification page gained a bold lead-in phrase. PEC gained bold on two sentences and one phrase plus its first two screenshot images, and its "72h Submitted under PEC" quote turned curly single. The FAQ page picked up eight more space-inside-`<strong>`-tag shifts (Q6, Q7 ×3, Q9 ×2, Q16, Q20) and one bold span split (Q7's Final Report bullet), all invisible in rendering.

**Unchanged**

Main page (Content navigation still 9 entries); FAQ Q1–Q31 wording ("Updated: 12 September 2026"); CSIRT list (27 countries, "Last updated: 10 September 2026"); Glossary (still v1.3, 39 fields, all rows re-verified) — see `enisa-srp-glossary-baseline.md`; AR User Manual, Terms and Conditions, and AR User Tutorial Video byte for byte; all four guidance pages' "Last updated" stamps unmoved.

### 2026-09-14 05:17 UTC (vs. 2026-09-14 04:13 UTC)

Two more markup-only edits, both on guidance subpages already mid-restructure; no wording, deadline, obligation, or field name changed anywhere. The FAQ page, main page, CSIRT list, AR Interface Functions, PEC, the Glossary, and the three newer pages are all unchanged.

**Editorial**

- **AR User Registration's "General Notes" turned from one flowing paragraph into a 4-item bulleted list**, each item now opening with a bold lead-in phrase ("**An EU Login account with multi-factor authentication (MFA) is needed**...", "**Validation by the CSIRT Designated as Coordinator (CDaC)...**", "**Manufacturers and open-source stewards are advised to...**"). Wording is byte-identical, including the tracked "ARs that have already" grammar slip — only the structure and emphasis are new. This is the one part of the page the 2026-09-14 03:15 UTC restructuring left untouched; it is now caught up with the rest of the page.
- **AR Notification Submission and Update picked up three small typographic changes**: the 72-hour/PEC sentence "...Particularly Exception Circumstances have not been invoked" gained a `<u>` underline on "not" (no wording change, and the Final Report page's parallel sentence still has no such underline); the "Optionally, fill in the Additional Notes" bullet lost its trailing full stop; and the Exceptions sentence's quoted "Update" (`click 'Update'`) now uses curly double quotes (`click "Update"`) instead of the straight single quotes this baseline had recorded.

**Unchanged**

FAQ Q1–Q31 (same tags and wording, "Updated: 12 September 2026"); main page (Content navigation still 9 entries, same order, all cards and dates); AR User Registration's Pre-conditions/Steps/Expected-result content otherwise byte for byte; AR Interface Functions and PEC guidance subpages byte for byte, still "Last updated: 12 September 2026" and "09 September 2026" respectively; AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; CSIRT list (27 countries, same links, "Last updated: 10 September 2026"); Glossary (still v1.3, 39 fields, all rows re-verified field by field) — see `enisa-srp-glossary-baseline.md`.

### 2026-09-14 04:13 UTC (vs. 2026-09-14 03:15 UTC)

A small italics/bold pass hit five spots on the FAQ page and one on the main page, plus a further wording pass on two of the three restructured guidance subpages (AR Notification Submission and Update, AR Interface Functions); AR User Registration, PEC, the Glossary, the CSIRT list, and the three newer pages (AR User Manual, Terms and Conditions, AR User Tutorial Video) are all unchanged. No deadline, obligation, numeric limit, or field name changed anywhere.

**New**

- **AR Notification Submission and Update gained two screenshot captions**: "Before CSIRT validation:" and "After CSIRT validation:", each followed by an image, inserted between the Early Warning Exceptions line and the start of "Submit a 72-hour Notification". No caption text beyond the two labels.

**Changed**

- **AR Notification Submission and Update's Early Warning and 72-hour Notification "Expected results" bullets were each rewritten** from one comma-chained sentence into two or three short sentences; the facts (who is emailed/alerted, when ENISA is copied, PEC's effect on the 72h case) are unchanged. The rewritten Early Warning sentence introduces a new grammar slip: "An email and an alert will also be sent to **the** ENISA automatically." — the 72-hour version's equivalent sentence has no such "the".
- **AR Interface Functions**: four sentences reworded, same substance in each case — "Invite Secondary AR"'s Purpose line gained "Follow these steps, as the Primary AR, to..."; "A pop-up confirming the invitation..." lost its leading "A" ("Pop-up confirming..."); "Search, Sort & Filter Notifications"' Purpose sub-line and its Steps were both expanded from one run-on sentence into an explicit "You can..." bullet list, adding phrasing ("with the following criteria", "if needed") without changing any criterion, column, or filter.

**Editorial**

Five markup-only changes on the FAQ page (new `<em>` italics wrapping the Art. 16(1) quote in Q2, the Commission-guidance subsection reference in Q11, and the "third-party component" clause in Q14; a stray trailing `<br>&nbsp;` inside Q7's Final Report/severe-incidents bullet; the page-level "Updated: 12 September 2026" stamp now italicised) and one on the main page (the Guidance section's disclaimer gained bold on its first two sentences, third left plain) — no wording changed in any of them.

**Unchanged**

FAQ Q1–Q31 otherwise (same tags and wording, "Updated: 12 September 2026"); main page otherwise (Content navigation still 9 entries, all cards and dates); AR User Registration and PEC guidance subpages byte for byte, still "Last updated: 12 September 2026" and "09 September 2026" respectively; AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; CSIRT list (27 countries, same links, "Last updated: 10 September 2026"); Glossary (still v1.3, 39 fields, all rows re-verified field by field) — see `enisa-srp-glossary-baseline.md`. AR Notification Submission and Update and AR Interface Functions "Last updated" stamps both unmoved at "12 September 2026" despite today's wording changes — continuing the pattern that these stamps are not a reliable change signal.

### 2026-09-14 03:15 UTC (vs. 2026-09-13 23:15 UTC)

Three of the four guidance subpages were restructured from flowing arrow-chained prose into a headed Pre-conditions/Steps/Expected-result format; no deadline, obligation, numeric limit, or field name changed anywhere. The FAQ page, main page, Glossary, CSIRT list, AR User Manual, Terms and Conditions, and AR User Tutorial Video are all unchanged since the last check.

**Changed**

- **AR User Registration, AR Notification Submission and Update, and AR Interface Functions rewritten into headed steps.** Each procedure's "Pre-conditions: A; B." and "Steps: click X → then Y → then Z." single sentences are now a `<h4>Pre-conditions</h4>` and `<h4>Steps</h4>` (or a bold lead-in), each condition/step its own bullet and full sentence. PEC, the fourth guidance page, was not touched and stays in its prior paragraph form. Facts are unchanged: still up to 20 Secondary ARs, up to 20 notifications while unverified, a 7-day invitation expiry, the same "Verified"/"Unverified" status names, and the same exception behaviour on every flow. None of the four pages' own "Last updated" stamps moved.
- **AR Interface Functions' "Invite Secondary AR" precondition lost its bold and outer quotes.** It had read in full bold, quoted: `"Your AR-manufacturer association has been validated by the CSIRT Designated as Coordinator (CDaC) and is displayed as 'Verified'."` It now reads as plain list text with only "Verified" itself quoted (curly quotes) — the same pattern already tracked for AR User Registration's analogous precondition since 2026-09-13 23:15 UTC, now confirmed on this page too.

**Watch**

- **New typo**: AR Notification Submission and Update's 72-hour PEC dissemination state now reads `"72h Submitted under PEC "` — a stray space before the closing quote mark, not present at the last check.
- The rewritten AR Interface Functions page uses three different spellings of the same label — "Pre-conditions" (hyphenated) on its first three procedures, "Preconditions" (no hyphen) from "Claim Primary AR Role" onward, and singular "Precondition" on the Dashboard section — all reproduced verbatim.

**Editorial**

One rephrasing each on AR Interface Functions' "Delete Association" precondition list and "View Alerts" section (same facts, reworded sentences); no new typos beyond the one noted above.

**Unchanged**

FAQ (Q1–Q31, same tags and wording, "Updated: 12 September 2026"); main page (Content navigation still 9 entries, all cards and dates matching); CSIRT list (27 countries, same links, "Last updated: 10 September 2026"); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; PEC guidance page byte for byte, still "Last updated: 09 September 2026"; Glossary (still v1.3, 39 fields, all rows re-verified field by field, including field 10's `cryptoprocessing&nbsp; devices` gap, v27's doubled full stop, v28's empty Example, and v30's missing space in "(CDaC)taking") — see `enisa-srp-glossary-baseline.md`.
...
AR Interface Functions' own "Last updated" stamp caught up to today's date with no wording change anywhere on the page, closing the gap with the other two rewritten-style guidance pages; nothing else — main page, FAQ, PEC guidance, CSIRT list, AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary — changed.

**Watch**

- AR Interface Functions' stamp moved "09 September 2026" → "12 September 2026" — the third of the four guidance pages to pick up a same-day stamp move today, after AR User Registration and AR Notification Submission and Update at 17:11 UTC. Word-for-word comparison against the full baseline text (all seven procedures: View Personal/Update Manufacturer Details, Invite Secondary AR, Add Additional Manufacturer Association, Claim Primary AR Role, Delete AR–Manufacturer Association, Dashboard, Search/Sort/Filter, View Notification Details, View Alerts) found zero text changes. The main page's own card for this guidance still reads "Updated: 9 September 2026", so the two now disagree — the same lagging-stamp pattern already tracked for the other three guidance cards.

**Unchanged**

Main page (Content navigation still 9 entries, Access button, all Resources/Guidance cards, Factsheet translations incl. the NL anomaly); FAQ Q1–Q31 wording and tags identical (5× [UPDATED]: Q4, Q7, Q17, Q22, Q29; "Updated: 12 September 2026"; Q1's `<strong>&nbsp;</strong>` and Q27's `Art. 14(<s>3</s>1)` artifacts intact); CSIRT list (27 countries, same links, "Last updated: 10 September 2026", Croatia's `<u>`-wrapped cell and disclaimer sentence as logged); AR User Registration and AR Notification Submission and Update (same stamps and text as the 17:11 UTC capture); PEC guidance (still "Last updated: 09 September 2026", same tracked defects); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; Glossary unchanged (still v1.3, 39 fields, field 10's Example still `cryptoprocessing&nbsp; devices`, field v28's Example still empty, the 42 "By default copied from previous step, or updated" cells, v27's doubled full stop, and all other field content re-verified — see `enisa-srp-glossary-baseline.md`).

### 2026-09-12 17:11 UTC (vs. 2026-09-12 13:10 UTC)

Two guidance pages' own "Last updated" stamps jumped to today with no wording change anywhere on either page, and the CSIRT list's disclaimer sentence was quoted in this baseline for the first time; nothing else — main page, FAQ, the other two guidance pages, AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary — changed.

**Watch**

- AR User Registration's stamp moved "10 September 2026" → "12 September 2026", and AR Notification Submission and Update's moved "09 September 2026" → "12 September 2026"; word-for-word comparison found zero text changes on either page. Both subpages now disagree with their own still-unmoved main-page card dates (10 and 9 September respectively) — the same lagging-stamp pattern flagged repeatedly since first capture. Per this file's own note, a stamp move is a prompt to diff, not a signal on its own, and the diff came back clean.

**Editorial**

The CSIRT list carries a disclaimer sentence — "By clicking on these links, you will be redirected to an external website. Please note that external websites are subject to their own terms and privacy policies." — sitting directly under the page's date stamp in the raw HTML. This file had never quoted it; added now for fidelity. Provenance unclear, same as the Croatia markup finding: whether it is new or simply unrecorded until now cannot be determined from this run alone.

**Unchanged**

Main page (Content navigation still 9 entries, all cards and links as logged); FAQ Q1–Q31 wording and tags identical to the 13:10 UTC capture ("Updated: 12 September 2026", Q1 and Q27 artifacts intact); CSIRT list otherwise identical (27 countries, same links, "Last updated: 10 September 2026", Croatia and Germany cells exactly as logged); AR Interface Functions and PEC guidance pages (same stamps, same tracked defects); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; Glossary unchanged (still v1.3, 39 fields, field 10's Example still `cryptoprocessing&nbsp; devices`, field v28's Example still empty, all other field content re-verified — see `enisa-srp-glossary-baseline.md`).
...
All eleven pages returned HTTP 200 and were confirmed as real page bodies. One markup-only anomaly, on the CSIRT list's Croatia row; everything else — main page, FAQ, all four guidance pages, AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary — unchanged.

**Editorial**

The CSIRT list's Croatia link cell reads `<td><a href="https://ncsc.hr/hr/kontakt"><u>https://ncsc.hr/hr/kontakt</u></a></td>` — wrapped in a `<u>` tag and missing the trailing `&nbsp;` that all other 26 country link cells carry. Every other row follows the uniform `<a href="...">...</a>&nbsp;` pattern; Croatia is now the only exception. Not previously called out at this precision: this file's own transcription of the Croatia link (last touched 2026-09-10 10:12 UTC, when the domain changed from `cert.hr` to `ncsc.hr`) has only ever quoted the URL, not the surrounding markup, so whether this is new this run or has stood unnoticed since that edit cannot be determined from this run alone. Same URL, same country, still 27 countries total; no link broken.

**Unchanged**

FAQ Q1–Q31 wording and tags identical ("Updated: 11 September 2026", Q1's `<strong>&nbsp;</strong>` artifact, Q27's `Art. 14(<s>3</s>1)` artifact, zero `<table>` elements on Q16, closing note still unlinked); CSIRT list otherwise identical (27 countries, same links, "Last updated: 10 September 2026", Germany's `<br>&nbsp;` cell still exactly as logged); all four guidance subpages (same stamps, same tracked defects); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; main page (Content navigation still 9 entries, Get Started/Resources cards, guidance-section dates all matching); Glossary unchanged (still v1.3, 39 fields, field 10's Example still `cryptoprocessing&nbsp; devices`, field v28's Example still empty, all other field content re-verified — see `enisa-srp-glossary-baseline.md`).
...
All eleven pages returned HTTP 200 and were confirmed as real page bodies. One markup-only change, on the CSIRT list; everything else — main page, FAQ, all four guidance pages, AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary — unchanged.

**Editorial**

The CSIRT list's Germany cell gained a trailing `<br>&nbsp;` after the country name (`<td>Germany<br>&nbsp;</td>`, was `<td>Germany</td>`); invisible in rendering, no link or country affected, still 27 countries.

**Unchanged**

FAQ Q1–Q31 wording and tags identical ("Updated: 11 September 2026", Q27's `Art. 14(<s>3</s>1)` artifact, zero `<table>` elements on Q16, closing note still unlinked); CSIRT list otherwise identical (27 countries, same links, "Last updated: 10 September 2026", no trailing full stop); all four guidance subpages (same stamps, same tracked defects, PEC's "Particularly"/"Particular" inconsistency and "Reason" fix both still in place); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; main page (Content navigation still 9 entries, Get Started/Resources cards, guidance-section dates all matching); Glossary unchanged (still v1.3, 39 fields, field 10's Example still `cryptoprocessing&nbsp; devices`, field v28's Example still empty, all other field content re-verified — see `enisa-srp-glossary-baseline.md`).

### 2026-09-12 07:12 UTC (vs. 2026-09-12 06:11 UTC)

All eleven pages returned HTTP 200 and were confirmed as real page bodies; nothing changed on the main page, the four guidance pages, the CSIRT list, the AR User Manual, Terms and Conditions, the AR User Tutorial Video page, or the Glossary. One markup-only change on the FAQ page, plus no new page in the "Content" navigation (still 9 entries, unchanged from baseline).

**Editorial**

One link removed: the closing note's "available supporting materials" (Q&A end-of-page note) is no longer hyperlinked — it now sits inside a plain `<strong>` tag with no `<a>`. The wording of the sentence is unchanged; see the closing-note entry below for the full text and the corrected history.

**Unchanged**

FAQ Q1–Q31 wording and tags otherwise identical ("Updated: 11 September 2026", Q27's `Art. 14(<s>3</s>1)` artifact, zero `<table>` elements on Q16); CSIRT list (27 countries, same links, "Last updated: 10 September 2026", no trailing full stop); all four guidance subpages (same stamps, same tracked defects); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; main page (Content navigation still 9 entries, Get Started/Resources cards, guidance-section dates all matching); Glossary unchanged (still v1.3, 39 fields, field 10's Example still `cryptoprocessing&nbsp; devices`, all other field content re-verified — see `enisa-srp-glossary-baseline.md`).

### 2026-09-12 06:11 UTC (vs. 2026-09-12 05:12 UTC)

All eleven pages returned HTTP 200 and were confirmed as real page bodies (title tags checked, full plain-text extraction diffed against this baseline, and the Glossary's raw `<table>` parsed cell by cell). Nothing changed on any of the ten pages tracked in this file; one finding, on the Glossary, corrects this file's own prior-hour entry rather than adding new content.

**Watch**

- The Glossary's field 10 (Product category) Example again shows the double-space-like gap before "devices" that this file's 05:12 UTC entry (above) reported as fixed — this time as `&nbsp;` plus a literal space rather than two plain spaces. Full detail in `enisa-srp-glossary-baseline.md`; nothing here needed updating since this file only carries the Glossary in trimmed summary form.

**Unchanged**

FAQ Q1–Q31 (same tags, "Updated: 11 September 2026", Q27's `Art. 14(<s>3</s>1)` artifact still present, zero `<table>` elements on Q16); CSIRT list (27 countries, same links, "Last updated: 10 September 2026", no trailing full stop); all four guidance subpages (same stamps, same tracked defects); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; main page (Content navigation still 9 entries, Get Started/Resources cards, guidance-section dates all matching); Glossary otherwise unchanged (still v1.3, 39 fields, all other field content re-verified).

### 2026-09-12 05:12 UTC (vs. 2026-09-12 01:13 UTC)

A page-wide terminology change on the Glossary plus two small fixes there; all ten other pages, including the CSIRT list, are byte-for-byte unchanged. Full detail in `enisa-srp-glossary-baseline.md`.

**Changed**

- **Glossary — 72h/Final Report status label expanded page-wide**: all 42 cells that read "copied-or-updated" across the Common, AEV, and SI tables now read "By default copied from previous step, or updated" instead. No status semantics changed, no field added/removed/renumbered, and the version footer did not move (still "Version 1.3. Last update: 10 September 2026").

**Fixed**

- **Glossary field 10 (Product category)**: the double-space typo in the Example text ("cryptoprocessing  devices"), tracked since 2026-09-10 13:11 UTC, is gone.

**Editorial**

One cell (Glossary field v28's Example) lost its "—" placeholder and is now genuinely empty; see `enisa-srp-glossary-baseline.md`.

**Unchanged**

FAQ Q1–Q31 (same tags, "Updated: 11 September 2026", Q27's `Art. 14(<s>3</s>1)` artifact still present, zero `<table>` elements on Q16); CSIRT list (27 countries, same links, "Last updated: 10 September 2026"); all four guidance subpages (same stamps, same tracked defects); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; main page (Content navigation still 9 entries); Glossary otherwise unchanged (still v1.3, 39 fields, all field names/meanings/how-to-complete/format/EW24h statuses re-verified).
...
The main page gained a direct access link to the live platform, timed with today's go-live; all ten other pages are byte-for-byte unchanged, including the Glossary's all-39-field table re-verified cell by cell.

**New**

- **"Access the CRA Single Reporting Platform" button**: directly under the "Get Started" heading, before "Resources", the main page now carries "Access the CRA Single Reporting Platform: https://portal.cra-srp.enisa.europa.eu/" as its own `<h2>` inside a `<blockquote>`, hyperlinked to the live production portal (trailing slash, unlike the FAQ's own Q9/Q28 citations of the same host). Not previously present in any capture of this section — the "Get Started" area had only ever been recorded as the "Resources" card group. Coincides with today's "CRA Single Reporting Platform is launched" press release already surfaced in "Related content".

**Unchanged**

FAQ Q1–Q31 (same tags, "Updated: 11 September 2026", Q27's `Art. 14(<s>3</s>1)` artifact still present, zero `<table>` elements on Q16); CSIRT list (27 countries, same links, "Last updated: 10 September 2026"); all four guidance subpages (same stamps, same tracked defects); AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; main page's Resources cards (Factsheet incl. the nine translations and the NL anomaly, new Terms and Conditions card, FAQ, Glossary) and User Support and Guidance section; Content navigation still 9 entries; Glossary (still v1.3, 39 fields, re-verified field by field against the raw `<table>`) — see `enisa-srp-glossary-baseline.md`.

### 2026-09-11 15:10 UTC (vs. 2026-09-11 14:13 UTC)

Two new items appeared in the main page's "Get Started → Resources" section; all other ten pages — FAQ, CSIRT list, the four guidance subpages, AR User Manual, Terms and Conditions, Tutorial Video, and the Glossary — are byte-for-byte unchanged. The Content navigation is still 9 entries, so neither item is a new page.

**New**

- **Factsheet translations**: the Factsheet card gained a new line, "Also available in: DE, EL, FR, HU, LV, NL, RO, SL, SV", each linking to a translated PDF under `/sites/default/files/2026-09/`. Previously the Factsheet was English-only ("currently in English"), and Q24 still states the platform itself will be English-only at launch — this is the factsheet document, not the platform UI.
- **Terms and Conditions download card**: a second, direct-download "Terms and Conditions" card ("Download the CRA - SRP Terms and Conditions") now sits in Resources, between the Factsheet and FAQ cards, linking straight to the same PDF the dedicated Terms and Conditions subpage (`terms_conditions_url`) already links to. That subpage itself is unchanged.

**Watch**

- Eight of the nine translated Factsheet filenames follow a consistent `ENISA_SRP_FACTSHEET_0N[_]XX.pdf` pattern (`01DE`, `02EL`, `03_FR`, `04_HU`, `05LV`, `07_RO`, `08_SL`, `09_SV` — note also `03_FR`/`04_HU` use an underscore before the language code where `01DE`/`02EL`/`05LV` don't). The **NL** link breaks the pattern outright: `ENISA_SRP_FACTSHEET_NL%20BE.pdf` — no numeric prefix, and "BE" where every other file has only a language code. Unclear whether this is a wrong/mislabelled file (Belgium-Dutch content under the Netherlands slot) or just an odd filename; worth a re-check once the file itself is worth opening.

**Editorial**

All nine new Factsheet-translation `href`s carry the same stray trailing space inside the attribute (e.g. `....pdf "`) that this file has previously tracked on other ENISA links.

**Unchanged**

FAQ Q1–Q31 (same tags and wording, "Updated: 11 September 2026", Q27's `Art. 14(<s>3</s>1)` artifact still present); CSIRT list (27 countries, same links, "Last updated: 10 September 2026"); all four guidance subpages; AR User Manual, Terms and Conditions, and AR User Tutorial Video pages byte for byte; main page intro and Content navigation (still 9 entries); Glossary (still v1.3, 39 fields, re-verified field by field against the raw `<table>`) — see `enisa-srp-glossary-baseline.md`.
...
**Since 2026-09-11 16:08 UTC**, directly under the "Get Started" heading and before "Resources", the page carries its own `<h2>` inside a `<blockquote>`: "Access the CRA Single Reporting Platform: [https://portal.cra-srp.enisa.europa.eu/](https://portal.cra-srp.enisa.europa.eu/)" — a direct, hyperlinked access button to the live production portal (with a trailing slash, unlike the host as cited in FAQ Q9/Q28). Not present at the 15:10 UTC check; coincides with today's "CRA Single Reporting Platform is launched" press release already surfacing in "Related content".
...
**Non-breaking-space pass, 2026-09-17 09:00 UTC**: 17 of the 31 questions (Q1, Q2, Q4, Q6, Q8, Q9, Q10, Q11, Q13, Q15, Q16, Q18, Q21, Q22, Q26, Q28, Q29) gained roughly 25 new `&nbsp;` insertions between this check and the 2026-09-15 06:11 UTC baseline — mostly sentence/paragraph-final, with five mid-sentence in Q1 alone. Each instance is quoted verbatim in place below. No wording, deadline, obligation, or tag changed in this pass; see the change log.
...
- **Vulnerabilities **contained in a product with digital elements; *(trailing space moved from outside to inside the `<strong>` tag 2026-09-14 09:14 UTC; no wording change.)*
...
- **Early Warning: **Without undue delay and in any case within **24 hours **of becoming aware of the actively exploited vulnerability or severe incident; *(trailing spaces after "Early Warning:" and "24 hours" moved inside their `<strong>` tags 2026-09-14 09:14 UTC; no wording change.)*

- **Actively Exploited Vulnerability/Severe Incident Notification: **Without undue delay and in any case within **72 hours** of becoming aware, providing general information and an initial assessment; *(trailing space after the label moved inside the `<strong>` tag 2026-09-14 09:14 UTC; "72 hours" untouched, still space outside; no wording change.)*

- **Final Report:**

- For **actively exploited** **vulnerabilities**: No later than **14 days** after a corrective or mitigating measure (e.g., patch) becomes available. *("or mitigating" inserted 2026-09-12 13:10 UTC; space before the colon dropped 2026-09-13 22:13 UTC, was "vulnerabilities :"; bold added throughout this bullet list 2026-09-13 23:15 UTC — deadline labels and figures now bold, no wording change; split into two separate `<strong>` tags, "actively exploited"/"vulnerabilities", 2026-09-14 09:14 UTC — the same pattern already seen in Q4's "reporting"/"obligations "; no wording change.)*

- For **severe incidents**: Within **1 month** after the 72-hour notification. *(space before the colon dropped 2026-09-13 22:13 UTC, was "incidents :"; a stray trailing `<br>&nbsp;` appeared inside this bullet's `<li>` 2026-09-14 04:13 UTC, invisible in rendering, no wording change.)*
...
### Q9. [UPDATED] How do I access and register on the SRP, and what are the roles of Primary and Secondary ARs? *(re-tagged 2026-09-18 06:12 UTC with a real, substantive wording change — see below; retitled 2026-09-09; had lost its "[UPDATED]" tag 2026-09-11 13:17 UTC in the wider tag sweep — see the "FAQ entries" note above; was "How is the platform accessible and how does the registration process work?")*
...
Assigned Representatives (ARs) of manufacturers and, once applicable, open-source software stewards must have an **EU Login account with multi-factor authentication (MFA) enabled **and use it to register on the SRP.&nbsp; *(bolded 2026-09-13 23:15 UTC; trailing space moved inside the `<strong>` tag 2026-09-14 09:14 UTC, no wording change. Gained a further nbsp right after "on the SRP.", 2026-09-17 09:00 UTC.)* An EU Login account can be created in advance at the following link: [https://ecas.ec.europa.eu/cas/login](https://ecas.ec.europa.eu/cas/login). No additional corporate entity authentication mechanism is currently used by the SRP.
...
Primary and Secondary ARs associated with the same manufacturer can access, view, and update notifications associated with that manufacturer, regardless of with AR originally submitted them. This allows another AR associated with the same manufacturer to continue the reporting process, including subsequent reporting stages and updates, as applicable. The only exception concerns draft notifications. Drafts are stored locally in the individual AR's account and are therefore not visible to other ARs associated with the same manufacturer. *(Rewritten 2026-09-18 06:12 UTC, replacing the single sentence tracked since 2026-09-09 — "The Primary AR can access all notifications associated with the manufacturer, whereas a Secondary AR can access only the notifications they submitted themselves." — with the two paragraphs quoted above. Substance change, not editorial: any AR on a manufacturer can now see and update every notification for that manufacturer, drafts excepted, where before a Secondary AR could see only their own submissions. "Regardless of with AR" is ENISA's own grammar slip, reproduced verbatim — presumably "regardless of which AR". This now contradicts the AR Interface Functions guidance page's "View the Dashboard" Expected result, itself unchanged today — see the change log.)*

A Secondary AR does not have the same administrative permissions as the Primary AR, but may claim the Primary AR role, subject to review and approval by the designated CSIRT. *(now its own paragraph, split off from the access-permission sentence above 2026-09-18 06:12 UTC; also gained a comma before "but", "as the Primary AR, but may claim"; no other wording change.)*
...
The manufacturer’s obligation to report actively exploited vulnerabilities is triggered when the manufacturer becomes aware of them. According to the European Commission’s “[FAQs on the CRA Implementation](https://ec.europa.eu/newsroom/dae/redirection/document/122331)” &nbsp;a manufacturer is not required to retrospectively report an actively exploited vulnerability where it was already aware of the active exploitation before 11 September 2026 (subsections 5.1 & 5.3). However, where the manufacturer becomes aware of the active exploitation after that date, the reporting obligation applies, including where the underlying vulnerability existed or was previously known.&nbsp; *(gained an nbsp right after the closing quote mark and a trailing nbsp at the paragraph's end, 2026-09-17 09:00 UTC; no wording change.)*
...
The Glossary also indicates whether a field is **required **(stemming directly from CRA obligations or identified by logical consequence), **optional**, **mandatory if the information is available**, or **carried forward** from a previous reporting stage and, when applicable, updated as additional information becomes available. *(the four status labels bolded 2026-09-13 23:15 UTC; "required"'s trailing space moved inside the `<strong>` tag 2026-09-14 09:14 UTC, no wording change.)*

The fields displayed and their requirements may differ depending on the notification type and reporting stage. ARs should complete all required fields and provide additional information whenever it is available and relevant.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)*
...
In general, you should report to the CDaC in the Member State of your main establishment in the EU. Under the CRA, the main establishment is the place where decisions related to the cybersecurity of your products with digital elements are predominantly taken.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)*
...
- **ENISA **(unless particularly exceptional circumstances apply). *(both bolded 2026-09-13 23:15 UTC; "ENISA"'s trailing space moved inside the `<strong>` tag 2026-09-14 09:14 UTC, no wording change.)*
...
The counters for the 72-hour Notification and Final Report are used as a reference for sending reminder emails and displaying alerts that a report may be overdue.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)*

These counters are intended to provide visibility but do not replace the responsibility of manufacturers and open-source software stewards to comply with the obligations laid down in CRA, including the requirement to report upon becoming aware, without undue delay and in any event within the timelines set out in Art. 14.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)*
...
- "CRA SRP Guidance - Particular Exceptional Circumstances (PEC)" — Updated: 10 September 2026 *(gained a date for the first time 2026-09-10 19:07 UTC; the dedicated subpage's own stamp is still "09 September 2026", unmoved — so this card now disagrees with its subpage the same way the AR User Registration card did until this run, just one day ahead instead of behind)*. "This section explains to ARs how and when to apply one of the three cases of Particularly Exceptional Circumstances (PEC), as described in the third subparagraph of Article 16(2) of the CRA." *(gained a card here 2026-09-10 10:12 UTC — previously listed only in the Content navigation, not in this section. The card's own link now carries a trailing `<br>&nbsp;` right after it, 2026-09-17 09:00 UTC; invisible in rendering, no wording change.)*
...
Page text, in full: "Please find the Terms and Conditions of the use of the SRP platform under this link:" followed, in a separate paragraph, by the bare link "[CRA Single Reporting Platform - Terms and Conditions](https://www.enisa.europa.eu/sites/default/files/2026-09/Terms%20and%20conditions%20CRA%20SRP%20v1.0%2020260910.pdf)" with no trailing full stop. *(Changed 2026-09-17 09:00 UTC: until the previous check this was one `<p>` — "...under this link: [CRA Single Reporting Platform - Terms and Conditions](url)." — a single sentence ending in a period right after the link. It is now split into two separate `<p>` elements: the lead-in sentence now ends in a colon with no period, and the link sits alone in its own paragraph, itself with no trailing period either. The link's target and visible text are unchanged.)* The page itself carries no further body text — it is a thin landing page pointing to the actual Terms and Conditions document, a PDF (`Terms and conditions CRA SRP v1.0 20260910.pdf`, not fetched/archived by this routine). The 10:12 capture recorded the target as `enisa.europa.eu/media/57353`; from 11:52 UTC at the latest the `href` is the direct file path. Between 11:52 and 13:11 UTC the `href` carried a trailing space inside the attribute, reproduced by ENISA's page verbatim; **as of 2026-09-10 14:07 UTC that trailing space is gone** — the attribute now closes cleanly right after `.pdf`. A future check should watch this page's own version/date stamp for a bump, and treat a changed `media/` link as a strong signal the underlying document itself changed.
...
- Croatia: https://ncsc.hr/hr/kontakt *(changed 2026-09-10 10:12 UTC from `https://www.cert.hr/en/home-page/` — different domain and authority name, from `cert.hr` to `ncsc.hr`. Markup noted 2026-09-12 11:11 UTC: the link cell is wrapped in `<u>` and lacks the trailing `&nbsp;` every other country's link cell has — `<a href="https://ncsc.hr/hr/kontakt"><u>https://ncsc.hr/hr/kontakt</u></a>`. Provenance unclear; not previously checked at this precision.)*
...
- Germany: https://www.bsi.bund.de/CERT-Bund/ ; https://www.bsi.bund.de/EN/CERT-Bund/ *(country-name cell gained a trailing `<br>&nbsp;` 2026-09-12 08:11 UTC — `<td>Germany<br>&nbsp;</td>`, was plain `<td>Germany</td>`; invisible in rendering, no link changed)*
...
**Structural rewrite, 2026-09-14 (first check of the day, 03:15 UTC)**: AR User Registration, AR Notification Submission and Update, and AR Interface Functions were each converted from the flowing "Pre-conditions: A; B." / "Steps: click X → then Y → then Z." arrow-chained-sentence style into a headed structure — a `<h4>Pre-conditions</h4>` (or a bold "Pre-conditions:" lead-in) followed by one `<li>` bullet per condition, and a `<h4>Steps</h4>` followed by one `<li>` bullet per step, each step now a complete sentence rather than an arrow-joined clause. PEC (the fourth page) was **not** restructured and stays in its prior flowing-paragraph form. None of the four pages' own "Last updated" stamps moved despite the rewrite on three of them — the same lagging-stamp pattern tracked since first capture. Substance is unchanged everywhere checked: the same numeric limits (up to 20 Secondary ARs, up to 20 notifications while unverified, 7-day invitation expiry), the same "Verified"/"Unverified" precondition and status names, and the same exception/error behaviour on every flow. Two things changed beyond pure restructuring, both noted inline below: AR Interface Functions' "Invite Secondary AR" precondition sentence lost its bold and its outer quotation marks (the same pattern already tracked for AR User Registration's analogous precondition since 2026-09-13 23:15 UTC — only "Verified" itself stays quoted, now with curly rather than straight quotes); and AR Notification Submission and Update's 72-hour PEC dissemination-state gained a stray space before its closing quote mark, `"72h Submitted under PEC "`. Screenshots (`<article>`/`<img>` elements) are present throughout but remain out of scope for this baseline, as before.

**Follow-up wording pass, 2026-09-14 04:13 UTC**: AR Notification Submission and Update gained two screenshot captions ("Before CSIRT validation:"/"After CSIRT validation:") and had its Early Warning and 72-hour "Expected results" bullets each split from one comma-chained sentence into short sentences (same facts; the Early Warning rewrite introduces a "the ENISA" grammar slip not present in the 72-hour version). AR Interface Functions had four sentences reworded — the "Invite Secondary AR" Purpose line, the pop-up confirmation sentence (lost its leading "A"), and the "Search, Sort & Filter Notifications" Purpose sub-line and Steps (expanded into an explicit list) — same substance throughout, no criterion, column, filter, limit, or status name changed. AR User Registration and PEC were untouched this pass. All noted inline below.

**Restructuring completed, 2026-09-14 10:19 UTC**: AR Interface Functions' remaining four flowing-prose sections ("View the Dashboard", "Search, Sort & Filter Notifications", "View Notification Details", "View Alerts") were converted into the same `<h4>Steps</h4>`-plus-bulleted-list format already applied to the page's other sections, completing the page-wide restructuring; two capitalisation/full-stop side effects noted inline below. AR Notification Submission and Update's "Update an Existing Notification" closing sentence split into two bullets, and three more Steps bullets across the page lost their trailing full stops. AR User Registration and PEC were untouched this pass. No wording, deadline, obligation, or field changed anywhere. All noted inline below.

**Non-breaking-space pass, 2026-09-17 09:00 UTC**: all four pages, including each page's own top disclaimer sentence (and PEC's Note sentence), gained new `&nbsp;` insertions since the 2026-09-15 06:11 UTC baseline — roughly 6 on AR User Registration, 10 on AR Notification Submission and Update, 14 on AR Interface Functions, and 5 on PEC, mostly sentence- or paragraph-final, plus a trailing empty `<p>&nbsp;</p>` at the end of AR User Registration and AR Interface Functions. Each instance is quoted verbatim in place below or in the disclaimer note above (the shared disclaimer sentence itself — "...applying these instructions." — gained a trailing nbsp on all four pages this run). Two further, non-nbsp changes this run, both on AR Interface Functions, are also noted inline below: the "Delete AR – Manufacturer Association" section's `<h4>Steps</h4>` heading now reads `<h4>Steps:</h4>` (colon added — the same colon addition this run hit AR Notification Submission and Update's Final Report section, see below; "View the Dashboard"'s own `<h4>Steps:</h4>` heading already carried this colon before today, tracked since 2026-09-14 10:19 UTC, and is unaffected), and the "Delete AR – Manufacturer Association" step's "Open the AR Association Management page..." sentence now splits across two `<strong>` tags, with the following "As a Primary AR..." sentence now sitting in its own paragraph between two bulleted lists rather than inside the first list item. No wording, deadline, obligation, or field changed anywhere.
...
**General Notes**:&nbsp; *(prose turned into a 4-item bulleted list with bold lead-ins 2026-09-14 05:17 UTC; wording unchanged, reproduced below as the live page now structures it. Gained a trailing nbsp after the label, 2026-09-17 09:00 UTC.)*
- AR users authenticate through EU Login when registering on the SRP. Authentication steps carried out within EU Login are not described here. Further information on EU Login is available through the official EU Login guidance at:&nbsp; https://trusted-digital-identity.europa.eu/index_en. *(gained an nbsp before the link, "guidance at:&nbsp; https://...", 2026-09-17 09:00 UTC; no wording change.)*
- **An EU Login account with multi-factor authentication (MFA) is needed** in order to access the platform. ARs that have already an EU Login account with no MFA enabled should enable MFA before their first access to the platform *(grammar as on the live page — "ARs that have already", reproduced verbatim; was "ARs who already have")*.
- **Validation by the CSIRT Designated as Coordinator (CDaC) of whether an AR is authorised to submit notifications on behalf of a specific manufacturer** takes place after registration (described below) and **is not a prerequisite for submitting a notification**. Validation takes place after the first access to the platform, in parallel with the reporting process, and will not affect the ability to submit notifications through the SRP. Specific validation procedures may vary between CSIRTs and remain the responsibility of the relevant CSIRT.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)*
...
**Pre-conditions** *(own `<h4>` heading since 2026-09-14; was a "Pre-conditions:" lead-in on a single semicolon-joined sentence)*:
- You are not already registered as an SRP user
- You have an active EU Login account with MFA enabled

**Steps** *(own `<h4>` heading since 2026-09-14; the single arrow-chained sentence below is now one `<li>` bullet per step, each a complete sentence — no wording change beyond that restructuring, noted once here rather than on every bullet)*:
- Open the SRP website (https://portal.cra-srp.enisa.europa.eu), select your AR role, and click “Continue” *(the parenthetical UI hint — "hovering over the 'i' icon shows information about each role" — is gone since the 2026-09-09 rewrite; straight quotes around "Continue" now curly, 2026-09-14 09:14 UTC)*.
- Select the CSIRT Designated as Coordinator (CDaC) from the drop-down menu and click “Continue”. Note: The manufacturer is responsible for identifying the correct CDaC in accordance with Article 14(7) of the CRA. *(straight quotes around "Continue" now curly, and the outer quotation marks around the "Note:" sentence gone, both 2026-09-14 09:14 UTC; no wording change.)*
- Authenticate through EU Login.
- Read and accept the legal agreement.
- Review and confirm the accuracy of your pre-filled personal details (First Name, Last Name, Email), then click “Continue”. These fields are retrieved from EU Login and cannot be edited in the SRP. *(straight quotes around "Continue" now curly, 2026-09-14 09:14 UTC; no wording change.)*
- Enter the manufacturer details (manufacturer name, additional information) and click “Continue” *(dropped "manufacturer address" 2026-09-10 13:11 UTC — the field previously listed here, and the long-tracked "adress" typo along with it, are both gone; this list still matches the two-item enumeration used in the Exceptions sentence below; straight quotes around "Continue" now curly, 2026-09-14 09:14 UTC)*.

Exceptions: If required manufacturer information is missing, such as Manufacturer Name, the SRP displays an error and the “Continue” button remains disabled. *(outer quotation marks around this sentence, and the straight single quotes around "Continue", both gone since 2026-09-14 09:14 UTC — "Continue" now curly double-quoted; no wording change.)*
Expected result *(relabelled "Expected result and system outcome" since 2026-09-14; no wording change)*: Registration is completed and the user account becomes “Active” with the “AR Primary User” role. The manufacturer entity is created in the SRP based on the information provided, and the AR-manufacturer association is submitted for validation by the CSIRT. The system sends a confirmation email to the user.&nbsp; The AR may start submitting notifications while validation of the AR–manufacturer association is pending. *(this closing sentence is now its own paragraph, outside the quoted "Expected result" block, since 2026-09-14; same wording. As of 2026-09-14 09:14 UTC the outer quotation marks around the "Registration is completed..." sentence are also gone, and the straight single quotes around 'Active' and 'AR Primary User' are now curly double quotes; no wording change. Gained a trailing nbsp after "the user.", 2026-09-17 09:00 UTC.)*
...
**Pre-conditions** *(own `<h4>` heading since 2026-09-14; was a semicolon-joined sentence)*:
- You are not already registered as an SRP user
- You have an active EU Login account with MFA enabled
...
Exceptions: If the invitation link has expired because more than 7 days have passed since the SRP sent the invitation, you will be redirected to a page displaying an error message. *(outer quotation marks around this sentence gone since 2026-09-14 09:14 UTC; no wording change.)*
- Review and confirm the accuracy of your pre-filled personal details (First Name, Last Name, Email) *(dropped "Legal name" 2026-09-10 13:11 UTC — matches the Primary AR flow's own three-field list above)* and click “Continue”. These fields are retrieved from EU Login and cannot be edited in the SRP.&nbsp; *(straight quotes around "Continue" now curly, 2026-09-14 09:14 UTC; no wording change. Gained a trailing nbsp 2026-09-17 09:00 UTC — the Primary AR flow's otherwise-identical sentence above did not gain one.)*
- Review and confirm the accuracy of the pre-filled manufacturer details for the manufacturer associated with the Primary AR who initiated the invitation and click “Accept invitation”. *(straight quotes around "Accept invitation" now curly, 2026-09-14 09:14 UTC; no wording change.)*

Expected result *(relabelled "Expected result and system outcome" since 2026-09-14; no wording change)*: Registration is completed and you become a Secondary AR for the relevant manufacturer. Your system role is displayed as “AR Backup User” and your status is set to “Active”. If registration is not completed within 7 days of the invitation being sent, the invitation expires and the user status becomes “Invitation Expired” in the SRP. *(outer quotation marks around this whole sentence, and the straight single quotes around 'AR Backup User', 'Active', and 'Invitation Expired', all gone since 2026-09-14 09:14 UTC — now curly double quotes; no wording change.)*

The page now ends with an empty `<p>&nbsp;</p>` after this sentence, 2026-09-17 09:00 UTC; no visible content.

### Guidance: CRA SRP - AR Notification submission and update
URL: see `guidance_urls`. Page note: "Last updated: 12 September 2026" *(stamp moved 2026-09-12 17:11 UTC from "09 September 2026", with zero wording change anywhere on the page; the main page's own card for this guidance still reads "Updated: 9 September 2026", so the two now disagree).*
...
*Second-person "Follow these steps…" style, as used on the other guidance pages. As of 2026-09-14, restructured further into headed Pre-conditions/Steps/Expected-result blocks per procedure (see the structural-rewrite note above); substance below is unchanged from the prior capture except where noted.*

**Submit a New Notification**
Purpose: Follow these steps to submit a new notification for an Actively Exploited Vulnerability (AEV) or a Severe Incident (SI). The purpose of the notification is to inform the relevant users of the CSIRT Designated as Coordinator (CDaC) so they can review, disseminate and further process it.
Pre-conditions: your user status is “Active”, you are logged in into the SRP and you have opened the Dashboard. *(grammar as on the live page — "logged in into", reproduced verbatim; new since 2026-09-14, previously "logged in". Straight quotes around "Active" now curly, 2026-09-14 09:14 UTC; no wording change.)*

*Submit an Early Warning* — Steps *(each formerly one arrow-chained clause, now one bullet per step since 2026-09-14; no wording change beyond splitting; **2026-09-14 09:14 UTC**: every step bullet across all three "Submit a..." Steps lists on this page (Early Warning, 72-hour Notification, Final Report) gained a bold lead-in phrase — "Click 'Submit new notification'"/"Fill the mandatory fields"/"Select an existing manufacturer"/"Click on an existing notification"/"Fill in the mandatory fields"/"Submit the notification"/"Submit the Final Report" are now `<strong>`-wrapped where each bullet begins; wording unchanged throughout, not annotated bullet-by-bullet below)*:
- Click “Submit new notification”. A new notification will be automatically created and pre-filled with the user type (assigned representative, AR). *(straight quotes now curly, 2026-09-14 09:14 UTC.)*
- Fill the mandatory fields and any optional fields needed to describe the notification under the Early Warning tab.
- Select an existing manufacturer (approved or pending validation) or add a new one.
- After filling in the required and optional fields, submit the notification or save it as a draft.
- Optionally, fill in the Additional Notes *(trailing full stop dropped 2026-09-14 05:17 UTC)*

&nbsp;<br>The notification is listed on the Dashboard. Exceptions: If you omit mandatory data, the SRP will return an error informing you of the missing data that must be provided.&nbsp; *(outer quotation marks gone, 2026-09-14 09:14 UTC; no wording change. As of 2026-09-17 09:00 UTC the "Dashboard." sentence is now preceded by its own `<p>&nbsp;<br>` — an nbsp-only paragraph line feeding into a `<br>` — and the Exceptions sentence gained a trailing nbsp; no wording change.)*

**New 2026-09-14 04:13 UTC**: two screenshot captions — "Before CSIRT validation:" and "After CSIRT validation:", each followed by an image — inserted here, between the Early Warning exceptions line and the start of "Submit a 72-hour Notification" below. No other text accompanies either caption.

*Submit a 72-hour Notification* — Pre-conditions:&nbsp; *(own bold `<strong>Pre-conditions:</strong>` paragraph, gained a trailing nbsp 2026-09-17 09:00 UTC)* your user status is “Active”, you are logged in into the SRP and you have opened the Dashboard; an Early Warning has already been submitted. *(**2026-09-14 09:14 UTC**: this is now a genuine 2-item `<li>` bulleted list on the live page — "Your user status is "Active", you are logged in into the SRP and you have opened the Dashboard" / "An Early Warning has already been submitted." — same two conditions, reproduced here unsplit as before; straight quotes around "Active" also now curly.)* Steps:
- Click on an existing notification on the Dashboard.
- Fill in the mandatory fields and any optional fields needed to describe the notification under the 72-hour Notification tab.
- Submit the notification or save it as a draft.

The notification is listed on the Dashboard and is subject to be validated by the CSIRT. Exception: If you omit mandatory data, the SRP will return an error informing you of the missing data that must be provided.&nbsp; *(outer quotation marks gone, 2026-09-14 09:14 UTC; no wording change; still singular "Exception:", unlike the Early Warning and Final Report Exceptions labels either side of it, which are plural. Gained a trailing nbsp 2026-09-17 09:00 UTC.)*

*Submit a Final Report* — Pre-conditions:&nbsp; *(own bold `<strong>Pre-conditions</strong>:` paragraph, gained a trailing nbsp 2026-09-17 09:00 UTC)* your user status is “Active”, you are logged in into the SRP and you have opened the Dashboard; an Early Warning and a 72-hour Notification have already been submitted. *(**2026-09-14 09:14 UTC**: this is now a genuine 2-item `<li>` bulleted list on the live page, same pattern and same two conditions as the 72-hour Notification precondition above, just unsplit here as before; straight quotes around "Active" also now curly.)* Steps: *(this section's own `<h4>Steps</h4>` heading now reads `<h4>Steps:&nbsp;</h4>` — colon and trailing nbsp both added 2026-09-17 09:00 UTC — unlike the plain `<h4>Steps</h4>` heading on the Early Warning and 72-hour Notification sections above, which are unchanged; no wording change.)*
- Click on an existing notification on the Dashboard.
- Fill in the mandatory fields and any optional fields needed to describe the notification under the Final Report tab.
- Submit the Final Report or save it as a draft *(trailing full stop dropped 2026-09-14 10:19 UTC — the equivalent Submit-flow bullets in the Early Warning and 72-hour Notification Steps lists above keep theirs)*

Exceptions: If you omit mandatory data, the SRP will return an error informing you of the missing data that must be provided. *(outer quotation marks gone, 2026-09-14 09:14 UTC; no wording change.)*

Expected results:
- Draft: the state of a saved draft notification is updated to "Draft". The draft is visible only to you.
- Early Warning: the notification is stored and is accessible to the CSIRT Designated as Coordinator (CDaC) and to ENISA. An email and an alert will be sent to this CSIRT. An email and an alert will also be sent to you and all other ARs of the selected manufacturer, confirming submission of the notification. An email and an alert will also be sent to the ENISA automatically. *(rewritten from one comma-chained sentence into three short sentences 2026-09-14 04:13 UTC; same facts, but the rewrite introduces a superfluous "the" before "ENISA" not present in the 72-hour bullet's equivalent sentence below — was "An email and an alert will be sent to this CSIRT, and also to you and all other ARs of the selected manufacturer, confirming submission, and automatically to ENISA.")* The state of a successfully submitted notification is updated to “Early Warning” *(straight quotes now curly, 2026-09-14 09:14 UTC; "72h Submitted", "Draft" and "FR Submitted" elsewhere in this Expected-results block stay straight; no wording change)*. Concerned CSIRTs will receive the Early Warning only after manual dissemination by the CSIRT Designated as Coordinator.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)*
- 72-hour Notification: The state of a successfully submitted notification is updated to "72h Submitted" *("the" capitalised to "The" 2026-09-14 19:10 UTC; the parallel Draft and Final Report bullets either side of this one keep lowercase "the state" after their own colon, so this bullet now breaks a three-way parallelism the baseline had recorded — no wording change)*. The dissemination state of a notification subject to Particularly **Exception** Circumstances (PEC) *(typo still present — "Exceptional" missing its "-al", reproduced verbatim; occurs three times in this section, same as before)* is updated to `"72h Submitted under PEC "` *(stray space before the closing quote mark, present since 2026-09-14 03:15 UTC — reproduced verbatim)*. An email and an alert will be sent to the CDaC. An email and an alert will also be sent to you and all other ARs of the selected manufacturer, confirming submission of the notification. An email and an alert will also be sent to ENISA. ENISA automatically receives the 72h Notification only when Particularly Exception Circumstances have <u>not</u> been invoked *(underline added to "not" 2026-09-14 05:17 UTC, no wording change; the Final Report bullet's parallel sentence below has no such underline)*. *(rewritten from one comma-chained sentence into four short sentences 2026-09-14 04:13 UTC; same facts, no "the ENISA" defect here unlike the Early Warning bullet above — was "An email and an alert will be sent to the CDaC, and also to you and all other ARs of the selected manufacturer, confirming submission, and to ENISA — but ENISA automatically receives the 72h Notification only when Particularly Exception Circumstances have not been invoked.")* Concerned CSIRTs will receive the 72-hour Notification only after manual dissemination by the CSIRT Designated as Coordinator.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)*
- Final Report: the state of a successfully submitted notification is updated to "FR Submitted". The sub-state of a successfully submitted Final Report is updated to “Final Report” *(straight quotes now curly, 2026-09-14 09:14 UTC; "FR Submitted" earlier in this bullet stays straight; no wording change)*. ENISA automatically receives the Final Report only when Particularly Exception Circumstances has not been invoked. Concerned CSIRTs will receive the Final Report only after manual dissemination by the CSIRT Designated as Coordinator.

**Update an Existing Notification** *(this `<h2>` heading gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change)*
...
Pre-conditions *(own heading since 2026-09-14, one bullet per condition; was one semicolon-joined sentence)*:
- Your user status is “Active" and you are logged in the SRP *(new defect 2026-09-14 09:14 UTC: the opening quote mark before "Active" is now curly while the closing one stays straight — a mismatched pair, reproduced verbatim; the analogous precondition on AR Interface Functions below has picked up the identical defect. No wording change.)*
- A notification has been previously submitted or saved as a draft
- Notification is not closed - closed notifications cannot be updated
- The Final Report is not submitted - the notification becomes non‑editable after the submission of a Final Report

Steps:
- Open a notification and update the necessary fields under the Notification tab *(trailing full stop dropped 2026-09-14 10:19 UTC)*
- Click ‘Update’ to save the updated notification information *(trailing full stop also dropped 2026-09-14 10:19 UTC; this bullet's straight double quotes around "Update" are now curly single quotes, 2026-09-14 09:14 UTC — the opposite style from the Exceptions sentence below, which stays curly double; no wording change.)*

Exceptions: If you omit mandatory data and click “Update”, the SRP will return an error informing you of the missing data that must be provided. *(curly double quotes around "Update" since 2026-09-14 05:17 UTC, was straight single quotes, click 'Update'; the outer quotation marks around this whole sentence are also gone as of 2026-09-14 09:14 UTC, no wording change.)*

The closing consequence sentence, until now one comma-joined sentence here, split into two bullet points 2026-09-14 10:19 UTC — same facts, subject repeated, first bullet losing its own full stop:
- The SRP automatically notifies the CDaC via an alert and an email
- The SRP automatically notifies ENISA and any concerned CSIRTs that previously received the notification through dissemination, via an alert and an email.

### Guidance: CRA SRP - AR Interface functions
URL: see `guidance_urls`. Page note: "Last updated: 12 September 2026" *(stamp moved 2026-09-12 18:10 UTC from "09 September 2026", with zero wording change anywhere on the page; the main page's own card for this guidance still reads "Updated: 9 September 2026", so the two now disagree — the same lagging-stamp pattern tracked since first capture).*

This section explains the AR interface of the SRP for various functions that can be performed. Note: AR users can access the Dashboard only after successfully logging in and while their SRP user status is “Active”, subject to applicable role and association restrictions. *(straight quotes around "Active" now curly, 2026-09-14 09:14 UTC; no wording change.)*

*As of 2026-09-14, each function below has its own heading with separate Pre-conditions/Steps/Expected-result blocks (see the structural-rewrite note above); one bullet per condition/step instead of one arrow-chained sentence. Substance unchanged except where noted.*

**Outer-quote removal and straight→curly quote pass, 2026-09-14 09:14 UTC**: every "Exceptions:"/"Expected result:" sentence on this page (and the same two sentence types on AR User Registration, AR Notification Submission and Update, and the Note sentence on PEC) that this baseline had reproduced wrapped in outer quotation marks now appears unquoted on the live page — the outer quotes are simply gone, wording unchanged in every case. Independently, most inline UI/status-name quotes on this page changed from straight to curly ("Send Invitation", "Association Management", "Add Manufacturer", "Save", "Continue", "Save Edit", "Deleted", "Read" among them); "Invite Backup" and the Add-Manufacturer step's "Settings" stayed straight. Two new defects came with this pass, both reproduced verbatim below: the Add-Manufacturer precondition's opening quote before "Active" is now curly while its closing quote stayed straight (a mismatched pair — the identical defect appeared on AR Notification Submission and Update's analogous precondition in this same check); and the Add-Manufacturer Expected result now quotes "Unverified" twice with two different styles in the same paragraph (curly on first use, straight on the second, previously straight both times).

**View Personal and Update Manufacturer Details through Settings**
Purpose: Follow these steps to view your personal details or update the details of a manufacturer. Pre-conditions: Your user status is “Active”, and you are logged in to the SRP. *(straight quotes around "Active" now curly, 2026-09-14 09:14 UTC; "your" capitalised to "Your" 2026-09-14 18:13 UTC, no other wording change.)* Steps:
- Open the Dashboard and click on your profile name. The profile drop-down menu opens.
- Click on the Settings menu. You can now view personal details or update manufacturer details by following the same steps used during the registration flow.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)* (separate "Personal details"/"Manufacturer details" screenshots, the second captioned "Manufacturer details:&nbsp;" — also a new trailing nbsp.)

**Invite Secondary AR (Primary AR only)**
Purpose: Follow these steps, as the Primary AR, to invite a Secondary AR for the same manufacturer to register on the SRP. *(reworded 2026-09-14 04:13 UTC to match the "Follow these steps..." phrasing used elsewhere on this page; was "as Primary AR, invite a Secondary AR for the same manufacturer to register on the SRP." — same substance.)* Pre-conditions:
- Your user status is set to “Active” and you are logged in to the SRP. *(straight quotes around "Active" now curly, 2026-09-14 09:14 UTC; no wording change.)*
- You have the Primary AR role for the relevant manufacturer.
- Your AR-manufacturer association has been validated by the CSIRT Designated as Coordinator (CDaC) and is displayed as "Verified". *(this precondition lost its bold and its outer quotation marks since 2026-09-14 — was **"Your AR-manufacturer association has been validated by the CSIRT Designated as Coordinator (CDaC) and is displayed as 'Verified'."** in full, wrapped in quotes and bold; now plain list-item text with only "Verified" itself quoted, and now with curly rather than straight quotes. Same wording, same substance — the "invitee's email not already registered" precondition from before the 2026-09-09/10 rewrites remains absent.)*

Steps:
- Open the Settings section and select the option to add a Secondary AR ("Invite Backup") for the manufacturer you represent. Note: You must first have a verified AR-manufacturer association before being able to invite a Secondary AR. *(this note, repeating the precondition, is unchanged.)*
- Enter the email address used by the prospective Secondary AR to register on EU Login and click “Send Invitation”.

Pop-up confirming the invitation has been sent is then displayed.&nbsp; *(dropped its leading "A" 2026-09-14 04:13 UTC — was "A pop-up confirming...". Gained a trailing nbsp 2026-09-17 09:00 UTC.)* Expected result: The SRP sends an email invitation to the Secondary AR. A new user record is created in the SRP with the specified email address and corresponding manufacturer details. The user role is assigned once the registration is completed. *(this precondition/expected-result sentence lost its own bold wrapping too, since 2026-09-14; wording unchanged — still correctly says the invitation goes to the Secondary AR, and the "Pending Invitation" status name is still not mentioned. Outer quotation marks around it also gone, 2026-09-14 09:14 UTC.)*

**Add Additional Manufacturer Association via Settings**
Purpose: Follow this procedure to request an additional manufacturer association through the Settings menu. Pre-conditions: Your user status is “Active" and you are logged in to the SRP. *(new defect 2026-09-14 09:14 UTC: opening quote before "Active" curly, closing quote stays straight — a mismatched pair, reproduced verbatim; the identical defect appeared on AR Notification Submission and Update's analogous precondition in this same check. "your" capitalised to "Your" 2026-09-14 18:13 UTC, no other wording change.)* Steps:
- Open the Dashboard, click on your profile name, and select "Settings" from the profile drop-down menu.
- Open “Association Management” and then click “Add Manufacturer”.
- Enter the required details for the new manufacturer and click on “Save”.

A pop-up confirms that the manufacturer has been created and associated with your account. Exceptions: If required manufacturer details are missing, such as&nbsp;Manufacturer Name, the SRP displays an error and the “Continue” or “Save Edit” button remains disabled. Expected result: The manufacturer record and the AR association with the manufacturer are created in the SRP with the status “Unverified”. A request to verify the AR-manufacturer association is submitted to the CSIRT Designated as Coordinator (CDaC) for validation. The SRP sends an email confirming the association request. As an "Unverified" AR, you can submit up to 20 notifications. *(this whole passage's outer quotation marks are gone, "Continue"/"Save Edit" are now curly, "the manufacturer record" now capitalised "The", and the first "Unverified" is now curly while the second stays straight — all 2026-09-14 09:14 UTC; a non-breaking space appeared before "Manufacturer Name" ("such as Manufacturer Name" → "such as&nbsp;Manufacturer Name") 2026-09-14 18:13 UTC, invisible when rendered; no wording change beyond the capitalisation and the nbsp.)*

**Claim Primary AR Role as a Secondary AR**
Purpose: Follow these steps, as a Secondary AR, to request to the Primary AR role for a manufacturer. *(the same missing-word grammar defect tracked since the 2026-09-09 rewrite, reproduced verbatim; outer quotation marks around this sentence gone since 2026-09-14 09:14 UTC.)* Pre-conditions *(own heading since 2026-09-14; from here down the page's own label spelling becomes inconsistent — "Pre-conditions" hyphenated above for View Personal/Invite Secondary AR/Add Manufacturer Association, "Preconditions" with no hyphen from this section onward, and "Precondition" singular on the Dashboard section below — all reproduced verbatim)*:
- You are a Secondary AR associated with the relevant manufacturer.
- Your user status is “Active”, and you are logged in to the SRP. *(straight quotes around "Active" now curly, 2026-09-14 09:14 UTC; no wording change.)*

Steps:
- Open the Dashboard, click your profile name, and select "Settings" from the profile drop-down list.
- Select the option to claim the Primary AR role for the relevant manufacturer.

A pop-up appears confirming the claim request. Expected result: The request is created and submitted to the designated CDaC for review and approval. The Secondary AR remains in the existing role until the request is approved. *(outer quotation marks around this sentence gone since 2026-09-14 09:14 UTC; no wording change.)*

**Delete AR – Manufacturer Association** *(this `<h2>` heading gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change)*
Purpose: Follow these steps to manage the association between AR users and manufacturers, including removal of your own association or, where applicable, the association of a Secondary AR. Preconditions:&nbsp; *(own bold `<strong>Preconditions</strong>:` paragraph, gained a trailing nbsp 2026-09-17 09:00 UTC)*
- Your user status is “Active” and are logged in to the SRP. *(grammar as on the live page — "and are logged in", reproduced verbatim; straight quotes around "Active" now curly, 2026-09-14 09:14 UTC, no wording change.)*
- You have a valid account with EU Login and are able to authenticate.
- The AR association with the manufacturer has been "Verified".

Steps: *(this section's own `<h4>Steps</h4>` heading now reads `<h4>Steps:</h4>` — colon added 2026-09-17 09:00 UTC; unlike "View the Dashboard"'s `<h4>Steps:</h4>` heading below, which has carried this colon since 2026-09-14 10:19 UTC, this is new; no wording change.)*
- Open the AR Association Management page in the SRP. *(as of 2026-09-17 09:00 UTC this step's own text now splits across two `<strong>` tags — "**Open the AR Association** **Management **page in the SRP." — and gained a trailing " &nbsp;"; same wording, invisible in rendering.)* As a Primary AR, you can remove your own association or remove a Secondary AR for the manufacturer. As a Secondary AR, you can remove your own association. *(previously one parenthetical inside the "open the page" step; now two separate sentences — same substance. As of 2026-09-17 09:00 UTC these two sentences sit in their own `<p>` between this list and the next one, rather than inside this `<li>`; same wording, structure only.)*
- Click "Delete Association" and confirm the action.

Expected result: The system removes the selected AR association, and its status is updated to “Deleted”. *(outer quotation marks around this sentence gone and the straight single quotes around 'Deleted' now curly double, 2026-09-14 09:14 UTC; no wording change.)*

**Dashboard**
Purpose: Use the Dashboard to view accessible notifications, search, sort and filter records, and open notification details. You can access the Dashboard by:
- opening it directly via a valid URL after login
- accessing the Dashboard URL while logged in
- selecting a Notification Alert, where available

Precondition *(rendered as a genuine 1-item bulleted list since 2026-09-14 12:22 UTC, was an inline sentence; same wording)*: Your user status is “Active”, and you are logged in to the SRP. *(straight quotes around "Active" now curly, 2026-09-14 09:14 UTC; "your" capitalised to "Your" 2026-09-14 18:13 UTC, no other wording change.)*

*View the Dashboard* — Purpose: Follow these steps to view the list of notifications accessible to you. Preconditions: Your user status is “Active” and you are logged in to the SRP. *(straight quotes around "Active" now curly, 2026-09-14 09:14 UTC; "your" capitalised to "Your" 2026-09-14 18:13 UTC, no other wording change.)* Steps *(own `<h4>Steps:</h4>` heading and single-item bullet since 2026-09-14 10:19 UTC, was a flowing "Steps: ..." sentence; same wording)*: Open the Dashboard. The content displayed depends on the notifications created by the logged-in AR. Expected result: A Primary AR can access all notifications associated with their manufacturer, whereas a Secondary AR can only access notifications they submitted and drafts they created. Secondary ARs cannot view notifications submitted by another AR associated with the same manufacturer.&nbsp; *(outer quotation marks gone, 2026-09-14 09:14 UTC; no wording change. Gained a trailing nbsp 2026-09-17 09:00 UTC. This page is unchanged as of 2026-09-18, but the FAQ's Q9 was rewritten that day to say the opposite — that a Secondary AR can see every notification on the manufacturer, drafts excepted — putting this sentence at odds with the FAQ; see the 2026-09-18 change log entry in this file.)*

*Search, Sort & Filter Notifications* — Purpose: Follow these steps to search, sort and filter the notifications accessible to you. The actions that you can perform include: Searching for notifications;&nbsp; Sorting notification records; and Filtering results based on specific criteria. *("The actions you can perform:" → "The actions that you can perform include:" 2026-09-14 04:13 UTC; same three actions, now capitalised as list items. The first bullet, "Searching for notifications;", gained a trailing nbsp 2026-09-17 09:00 UTC.)* Preconditions: your user status is “Active”; you are logged in to the SRP; at least one notification has previously been submitted or saved as a draft. *(**2026-09-14 09:14 UTC**: this semicolon-joined sentence is now a genuine 3-item `<li>` bulleted list on the live page — "Your user status is "Active"."/"You are logged in to the SRP."/"At least one notification has previously been submitted or saved as a draft." — same three conditions, reproduced here unsplit as before; straight quotes around "Active" also now curly.)* Steps *(own `<h4>Steps</h4>` heading since 2026-09-14 10:19 UTC, was a flowing "Steps: ..." sentence; the criteria/columns/filters below are now genuine nested `<li>` sub-bullets under top-level step bullets — "Access the Dashboard" / "You can use the 'Search' function with the following criteria: [sub-list]" / "You can sort by either of the following columns: [sub-list]" / "You can filter notifications by selecting 'All filters'" / "You can narrow down the results using one or more of the following criteria: [sub-list]" / "You can select the appropriate tab..." / "You can select 'Clear all filters'..." — same items, same wording, listed here unsplit as before)*: Access the Dashboard. You can use the "Search" function with the following criteria:&nbsp; Notification ID&nbsp; / Manufacturer&nbsp; / Title. *(gained three nbsp — after "criteria:", after "Notification ID", and after "Manufacturer" — 2026-09-17 09:00 UTC; no wording change.)* You can sort by either of the following columns: Title: alphabetical order / Last Update: chronological order. You can filter notifications by selecting “All filters”. *(straight quotes around "All filters" now curly, 2026-09-14 09:14 UTC; "Search" stays straight; no wording change.)* You can narrow down the results using one or more of the following criteria: Member States in which the product with digital elements is available / Type of submission. You can select the appropriate tab to focus on notifications with a pending action label.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC.)* *(**Removed 2026-09-14 09:14 UTC**: the parenthetical "(the "Action Required" tab is still not named by that label)" that had followed this sentence since the 2026-09-14 04:13 UTC rewrite is gone — the tab's actual on-screen name is not stated anywhere on the page now, where before the page at least noted that its own "Action Required" label doesn't appear on the tab itself. Not a restoration of the old defect, a different, smaller one: the page previously over-specified by naming a label it didn't use; now it under-specifies by not naming the tab at all.)* You can select “Clear all filters”, if needed, to remove all applied filters. *(the single semicolon-chained "access the Dashboard; search by...; sort by...; filter by..." sentence expanded 2026-09-14 04:13 UTC into this explicit "You can..." list; the same criteria, columns, and filters — Notification ID/Manufacturer/Title, Title/Last Update, "All filters", the two narrowing criteria, "Clear all filters" — are all still there, just phrased out in full rather than run together.)* Exceptions: In case no results are found, the search returns no results, and the system informs you that no notifications match the search criteria. *(singular "Exception:" is now plural "Exceptions:", 2026-09-14 09:14 UTC — matches every other guidance-page label on this page, which are all already plural; outer quotation marks around the sentence also gone.)* Expected result: The results are filtered and/or sorted based on your selections. *(outer quotation marks gone, 2026-09-14 09:14 UTC; no wording change.)*
...
This section explains to ARs how and when to apply one of the three cases of Particularly Exceptional Circumstances (PEC), as described in the third subparagraph of Article 16(2) of the CRA — this opening sentence still spells the term "Particularly". Disclaimer: The information on this page is provided according to our current best knowledge and may be subject to change. Please ensure you consult the latest available guidance and related legislation, before applying these instructions.&nbsp; Note: AR users can access the SRP Dashboard only after successfully logging in and while their SRP user status is "Active", subject to applicable role and association restrictions. *(**2026-09-14 09:14 UTC**: both the Disclaimer and Note sentences lost the outer quotation marks this baseline had reproduced around them; the Disclaimer sentence is also newly wrapped in `<strong>` bold, which it did not have before; the Note sentence's straight single quotes around 'Active' are now curly double. No wording change to either sentence. The Disclaimer sentence gained a trailing nbsp 2026-09-17 09:00 UTC.)*

*No `<h3>`/`<h4>` headings on the live page at any point — this file's own "Flagging a submission" / "Effect" labels below are this baseline's summarising labels, not page headings, and are kept only to structure the diff; the live page itself has never been a bulleted list, only flowing `<p>` paragraphs, matching the "PEC ... was not restructured and stays in its prior flowing-paragraph form" note above.*

**Particular Exceptional Circumstances (PEC)** *(this specific phrase gained `<strong>` bold 2026-09-14 09:14 UTC; no wording change)* are applicable only when a manufacturer or open-source software steward (hereinafter the "Reporter") submits a 72-hour Notification of&nbsp;an Actively Exploited Vulnerability (AEV). *(New defect in the 2026-09-09 22:12 UTC rewrite: this instance and the two below now read "**Particular** Exceptional Circumstances", dropping "-ly" — but the opening sentence above still reads "**Particularly** Exceptional Circumstances", so the page is now internally inconsistent about the spelling of its own defined term. Gained an nbsp, "Notification of&nbsp;an", 2026-09-17 09:00 UTC.)* **During the first 72-hour window, the Reporter should assess, where applicable, whether Particular Exceptional Circumstances (PEC) apply.**&nbsp; *(this whole sentence gained `<strong>` bold 2026-09-14 09:14 UTC; no wording change. Gained a trailing nbsp 2026-09-17 09:00 UTC.)*

- **Flagging a submission**: In order to flag a submission under PEC, in the 72-hour report template, scroll down and toggle the Particular Exceptional Circumstances (PEC) indicator. *(a screenshot image now follows this sentence, first seen on this page 2026-09-14 09:14 UTC — the same out-of-scope `<article>`/`<img>` pattern already tracked on the other three guidance pages.)* PEC can be invoked only where at least one of the conditions under the third subparagraph of Article 16(2) of the CRA applies. By toggling the indicator, the PEC Delay Reason option will be displayed. An optional field is also available to provide a justification that can help the CSIRT Designated as Coordinator (CDaC) decide whether to accept the submission under PEC. *(a second screenshot image follows here too, same as above.)*
- **Effect**: PEC is intended for exceptional situations in which the dissemination of information may need to be delayed to avoid security-related risks. **If PEC is invoked in the 72-hour Notification, the dissemination status becomes ‘72h Submitted under PEC’.**&nbsp; *(this whole sentence gained `<strong>` bold 2026-09-14 09:14 UTC, and the quotes around the phrase changed from straight double to curly single — no trailing-space defect here, unlike the analogous phrase on AR Notification Submission and Update; no wording change. Gained a trailing nbsp 2026-09-17 09:00 UTC.)* The CDaC remains responsible for deciding whether dissemination is necessary and possible, and for manually sharing the notification with other concerned CSIRTs and the full notification with ENISA.&nbsp; *(gained a trailing nbsp 2026-09-17 09:00 UTC; no wording change.)* In this scenario, the full notification is not simultaneously made available to ENISA. Only limited information in accordance with Article 16(2) is made available to ENISA.
...
- 2026-09-11 (14:13 UTC): checked all eleven tracked pages again with `curl --retry`-style backoff; all HTTP 200, confirmed as real page bodies. One real change: FAQ Q7's opening sentence typo ("actively exploitation vulnerability") fixed to "actively exploited vulnerability". All other ten pages, including the Glossary (still v1.3, 39 fields, re-verified field by field), byte for byte unchanged from the 13:17 UTC baseline. `last_check` and `last_change` both already 2026-09-11.
- 2026-09-11 (15:10 UTC): checked all eleven tracked pages again with `curl --retry`-style backoff against the frontmatter's own URL list; all HTTP 200, confirmed as real page bodies via title-tag checks and a full raw-HTML text extraction, including a structured table-cell parse of the Glossary's all-39-field table against `enisa-srp-glossary-baseline.md`. Walking the "Content" navigation again found the same 9 entries — no new page. Two real changes, both on the main page's "Get Started → Resources" section: the Factsheet card gained nine translated-PDF links ("Also available in: DE, EL, FR, HU, LV, NL, RO, SL, SV"), and a new direct-download "Terms and Conditions" card was added alongside it, linking to the same PDF the dedicated Terms and Conditions subpage already links to. FAQ (Q1–Q31, same tags, "Updated: 11 September 2026"), CSIRT list (27 countries, same links), all four guidance subpages, AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary (still v1.3, 39 fields, every previously tracked defect present unchanged) all otherwise byte for byte unchanged. `last_check` and `last_change` both already 2026-09-11.
- 2026-09-11 (16:08 UTC): checked all eleven tracked pages again with `curl --retry`-style backoff against the frontmatter's own URL list; all HTTP 200, confirmed as real page bodies via title-tag checks and a full raw-HTML text extraction, including a structured table-cell parse of the Glossary's all-39-field table against `enisa-srp-glossary-baseline.md`. Walking the "Content" navigation again found the same 9 entries — no new page. One real change: the main page gained a direct "Access the CRA Single Reporting Platform" button/heading, linking to the live production portal, directly under "Get Started" and before "Resources" — timed with today's go-live and the "CRA Single Reporting Platform is launched" press release already surfacing in "Related content". FAQ (Q1–Q31, same tags, "Updated: 11 September 2026", zero `<table>` elements on Q16), CSIRT list (27 countries, same links, "Last updated: 10 September 2026"), all four guidance subpages, AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary (still v1.3, 39 fields, every previously tracked defect present unchanged, all footnotes intact) all otherwise byte for byte unchanged from the 15:10 UTC baseline. See the change log at the top of this file. `last_check` and `last_change` both already 2026-09-11.
- (checks between 2026-09-11 16:08 UTC and 2026-09-12 10:13 UTC are recorded only in the change log at the top of this file, not repeated here.)
- 2026-09-12 (11:11 UTC): checked all eleven tracked pages with `curl --retry`-style backoff against the frontmatter's own URL list; all HTTP 200, confirmed as real page bodies via title-tag checks, a structured accordion-item parse of all 31 FAQ entries, and a structured table-cell parse of the Glossary's all-39-field table against `enisa-srp-glossary-baseline.md`. Walking the "Content" navigation again found the same 9 entries — no 12th page. One markup-only anomaly found: the CSIRT list's Croatia row is wrapped in a `<u>` tag and is missing the trailing `&nbsp;` every other of the 26 country link cells carries — not previously checked at this precision, provenance (new this run vs. present since the 2026-09-10 domain change) not determinable. FAQ (all 31 entries, same tags including Q1's `<strong>&nbsp;</strong>` and Q27's `<s>3</s>1` artifacts, "Updated: 11 September 2026"), CSIRT list otherwise unchanged (27 countries, same links, "Last updated: 10 September 2026", Germany's `<br>&nbsp;` cell unchanged), all four guidance subpages, AR User Manual, Terms and Conditions, AR User Tutorial Video, main page (9-entry Content nav, all cards and the Access button), and the Glossary (still v1.3, 39 fields, every previously tracked defect present unchanged) all byte for byte unchanged from the 10:13 UTC baseline. See the change log at the top of this file. `last_check` and `last_change` both already 2026-09-12.
- (checks between 2026-09-12 11:11 UTC and 2026-09-13 23:15 UTC are recorded only in the change log at the top of this file, not repeated here.)
- 2026-09-14 (03:15 UTC): first check of the day. Fetched all eleven tracked pages with `curl --retry`-style backoff against the frontmatter's own URL list; all HTTP 200, confirmed as real page bodies via title-tag checks, a structured accordion-item parse of all 31 FAQ entries, and a structured heading/list parse of all four guidance subpages, cross-checked against a full raw-HTML extraction. Walking the "Content" navigation again found the same 9 entries — no 12th page. Three of the four guidance subpages were restructured into headed Pre-conditions/Steps/Expected-result blocks; see the change log at the top of this file for the substantive findings (no obligation, deadline, or numeric limit changed). FAQ (Q1–Q31, same tags and wording, "Updated: 12 September 2026"), main page, CSIRT list, PEC guidance, AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary (still v1.3, 39 fields, re-verified field by field including the rowspan-split PEC Delay Reason cell) all otherwise byte for byte unchanged from the 23:15 UTC baseline. `last_check` and `last_change` both updated to 2026-09-14.
- (checks between 2026-09-14 03:15 UTC and 05:17 UTC are recorded only in the change log at the top of this file, not repeated here.)
- 2026-09-14 (09:14 UTC): checked all eleven tracked pages with `curl --retry`-style backoff against the frontmatter's own URL list; all HTTP 200, confirmed as real page bodies via title-tag checks and a full raw-HTML text extraction (plain-text extraction via Python's `html.parser` plus targeted `<strong>`/quote-character checks, cross-checked directly against the raw HTML for every flagged spot), including a structured table-cell parse of the Glossary's all-39-field table against `enisa-srp-glossary-baseline.md`. Walking the "Content" navigation again found the same 9 entries — no 12th page. One real content change (AR Interface Functions dropped the "Action Required" tab-naming parenthetical) plus a wide straight→curly quote conversion and outer-quote removal across all four guidance pages, two new mismatched-quote defects, a doubled-style "Unverified", and eight more space-inside-`<strong>` shifts on the FAQ page — see the change log at the top of this file for full detail and exact quotes. Main page (9-entry Content nav, all cards), CSIRT list (27 countries, "Last updated: 10 September 2026"), AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary (still v1.3, 39 fields, every previously tracked defect present unchanged) all otherwise byte for byte unchanged. `last_check` and `last_change` both already 2026-09-14.
- (checks between 2026-09-14 09:14 UTC and 2026-09-17 09:00 UTC, including the cadence switch from hourly back to weekly, are recorded only in the change log at the top of this file, not repeated here.)
- 2026-09-18 (06:12 UTC): first check of the day (the routine is back to weekly; `last_check` on `main` still read 2026-09-17). Fetched all eleven tracked pages with `curl --retry`-style backoff against the frontmatter's own URL list; all HTTP 200, confirmed as real page bodies via title-tag checks and a full raw-HTML text extraction, including a structured accordion-item parse of all 31 FAQ entries and a structured table-cell parse of the Glossary's all-39-field table against `enisa-srp-glossary-baseline.md`. Walking the "Content" navigation again found the same 9 entries — no 12th page. One real, substantive change, isolated to FAQ Q9: its notification-access paragraph was rewritten to say any AR on a manufacturer (Primary or Secondary) can now access, view and update all of that manufacturer's notifications except the other AR's drafts — replacing the previous rule that a Secondary AR could see only what they themselves submitted; Q9 regained its "[UPDATED]" tag and the FAQ page-level stamp moved to "Updated: 17 September 2026" for the first time since 12 September. This new wording contradicts the AR Interface Functions guidance page's own "View the Dashboard" section, itself unchanged and still describing the old, more restrictive access rule — flagged as a Watch item, not resolved by this baseline. One further stray nbsp turned up on Q7, outside the 2026-09-17 nbsp pass. Main page, FAQ Q1–Q8/Q10–Q31 and the closing note, CSIRT list (27 countries, same links, "Last updated: 10 September 2026"), all four guidance subpages otherwise (same stamps, same tracked defects), AR User Manual, Terms and Conditions, AR User Tutorial Video, and the Glossary (still v1.3, 39 fields, every previously tracked defect present unchanged) all otherwise byte for byte unchanged from the 2026-09-17 09:00 UTC baseline. See the change log at the top of this file for full detail and exact quotes. `last_check` and `last_change` both updated to 2026-09-18.
