...
retrieved: 2026-09-14 (fetched 05:17 UTC, unchanged since 04:13 UTC — see change log)
guidance_retrieved: 2026-09-14 (fetched 05:17 UTC, AR User Registration's General Notes went from prose to a bulleted list and AR Notification Submission and Update gained a new underline and two punctuation/quote-style tweaks, all markup-only; AR Interface Functions and PEC unchanged — see change log)
glossary_retrieved: 2026-09-14 (fetched 05:17 UTC, unchanged since 23:15 UTC on 2026-09-13 — see `enisa-srp-glossary-baseline.md`)
csirt_list_retrieved: 2026-09-14 (fetched 05:17 UTC, unchanged since 17:11 UTC on 2026-09-12)
ar_user_manual_retrieved: 2026-09-14 (fetched 05:17 UTC, wording unchanged since first capture at 10:12 UTC on 2026-09-10)
terms_conditions_retrieved: 2026-09-14 (fetched 05:17 UTC, unchanged since the trailing space was lost at 14:07 UTC on 2026-09-10)
ar_user_tutorial_video_retrieved: 2026-09-14 (fetched 05:17 UTC, unchanged since first capture at 11:10 UTC on 2026-09-11)
...
- Open the SRP website (https://portal.cra-srp.enisa.europa.eu), select your AR role, and click "Continue" *(the parenthetical UI hint — "hovering over the 'i' icon shows information about each role" — is gone since the 2026-09-09 rewrite)*.
- Select the CSIRT Designated as Coordinator (CDaC) from the drop-down menu and click "Continue". Note: "The manufacturer is responsible for identifying the correct CDaC in accordance with Article 14(7) of the CRA."
- Authenticate through EU Login.
- Read and accept the legal agreement.
- Review and confirm the accuracy of your pre-filled personal details (First Name, Last Name, Email), then click "Continue". These fields are retrieved from EU Login and cannot be edited in the SRP.
- Enter the manufacturer details (manufacturer name, additional information) and click "Continue" *(dropped "manufacturer address" 2026-09-10 13:11 UTC — the field previously listed here, and the long-tracked "adress" typo along with it, are both gone; this list still matches the two-item enumeration used in the Exceptions sentence below)*.

Exceptions: "If required manufacturer information is missing, such as Manufacturer Name, the SRP displays an error and the 'Continue' button remains disabled."
...
Exceptions: "If the invitation link has expired because more than 7 days have passed since the SRP sent the invitation, you will be redirected to a page displaying an error message."
- Review and confirm the accuracy of your pre-filled personal details (First Name, Last Name, Email) *(dropped "Legal name" 2026-09-10 13:11 UTC — matches the Primary AR flow's own three-field list above)* and click "Continue". These fields are retrieved from EU Login and cannot be edited in the SRP.
- Review and confirm the accuracy of the pre-filled manufacturer details for the manufacturer associated with the Primary AR who initiated the invitation and click "Accept invitation".

Expected result *(relabelled "Expected result and system outcome" since 2026-09-14; no wording change)*: "Registration is completed and you become a Secondary AR for the relevant manufacturer. Your system role is displayed as 'AR Backup User' and your status is set to 'Active'. If registration is not completed within 7 days of the invitation being sent, the invitation expires and the user status becomes 'Invitation Expired' in the SRP."
...
Pre-conditions: your user status is "Active", you are logged in into the SRP and you have opened the Dashboard. *(grammar as on the live page — "logged in into", reproduced verbatim; new since 2026-09-14, previously "logged in".)*

*Submit an Early Warning* — Steps *(each formerly one arrow-chained clause, now one bullet per step since 2026-09-14; no wording change beyond splitting)*:
- Click "Submit new notification". A new notification will be automatically created and pre-filled with the user type (assigned representative, AR).
...
The notification is listed on the Dashboard. Exceptions: "If you omit mandatory data, the SRP will return an error informing you of the missing data that must be provided."
...
*Submit a 72-hour Notification* — Pre-conditions: your user status is "Active", you are logged in into the SRP and you have opened the Dashboard; an Early Warning has already been submitted. Steps:
...
The notification is listed on the Dashboard and is subject to be validated by the CSIRT. Exception: "If you omit mandatory data, the SRP will return an error informing you of the missing data that must be provided."

*Submit a Final Report* — Pre-conditions: your user status is "Active", you are logged in into the SRP and you have opened the Dashboard; an Early Warning and a 72-hour Notification have already been submitted. Steps:
- Click on an existing notification on the Dashboard.
...
- Final Report: the state of a successfully submitted notification is updated to "FR Submitted". The sub-state of a successfully submitted Final Report is updated to "Final Report". ENISA automatically receives the Final Report only when Particularly Exception Circumstances has not been invoked. Concerned CSIRTs will receive the Final Report only after manual dissemination by the CSIRT Designated as Coordinator.
...
- Your user status is "Active" and you are logged in the SRP
...
Purpose: Follow these steps to view your personal details or update the details of a manufacturer. Pre-conditions: your user status is "Active", and you are logged in to the SRP. Steps:
...
A pop-up appears confirming the claim request. Expected result: "The request is created and submitted to the designated CDaC for review and approval. The Secondary AR remains in the existing role until the request is approved."
...
- Your user status is "Active" and are logged in to the SRP. *(grammar as on the live page — "and are logged in", reproduced verbatim.)*
...
Expected result: "The system removes the selected AR association, and its status is updated to 'Deleted'."
...
Precondition: your user status is "Active", and you are logged in to the SRP.

*View the Dashboard* — Purpose: Follow these steps to view the list of notifications accessible to you. Preconditions: your user status is "Active" and you are logged in to the SRP. Steps: Open the Dashboard. The content displayed depends on the notifications created by the logged-in AR. Expected result: "A Primary AR can access all notifications associated with their manufacturer, whereas a Secondary AR can only access notifications they submitted and drafts they created. Secondary ARs cannot view notifications submitted by another AR associated with the same manufacturer."

*Search, Sort & Filter Notifications* — Purpose: Follow these steps to search, sort and filter the notifications accessible to you. The actions that you can perform include: Searching for notifications; Sorting notification records; and Filtering results based on specific criteria. *("The actions you can perform:" → "The actions that you can perform include:" 2026-09-14 04:13 UTC; same three actions, now capitalised as list items.)* Preconditions: your user status is "Active"; you are logged in to the SRP; at least one notification has previously been submitted or saved as a draft. Steps: Access the Dashboard. You can use the "Search" function with the following criteria: Notification ID / Manufacturer / Title. You can sort by either of the following columns: Title: alphabetical order / Last Update: chronological order. You can filter notifications by selecting "All filters". You can narrow down the results using one or more of the following criteria: Member States in which the product with digital elements is available / Type of submission. You can select the appropriate tab to focus on notifications with a pending action label (the "Action Required" tab is still not named by that label). You can select "Clear all filters", if needed, to remove all applied filters. *(the single semicolon-chained "access the Dashboard; search by...; sort by...; filter by..." sentence expanded 2026-09-14 04:13 UTC into this explicit "You can..." list; the same criteria, columns, and filters — Notification ID/Manufacturer/Title, Title/Last Update, "All filters", the two narrowing criteria, "Clear all filters" — are all still there, just phrased out in full rather than run together.)* Exception: "In case no results are found, the search returns no results, and the system informs you that no notifications match the search criteria." Expected result: "The results are filtered and/or sorted based on your selections."

*View Notification Details* — Purpose: Follow these steps to view the details of notifications accessible to you. Steps: click on a notification to view its details. Expected result: "you can view the notification details."

**View Alerts**
Purpose: Follow these steps to review alerts generated by the system. Preconditions: you are registered as a user in the SRP; you have a valid account with EU Login (for AR) and can authenticate successfully. Steps: open the Alerts tab. Alerts are triggered by an action that requires the attention of the AR user. Unread alerts appear in blue *(rephrased from "'blue' = unread"; same fact)*. When an alert is opened, it is marked as "read" (turns grey) *(reordered from "turns grey ('read')"; same fact)*. Red alerts appear only when an exceptional or critical action has occurred (e.g., a designated CSIRT has invalidated a submission). Expected result: "The selected alert is marked as 'Read', and you can review its details."
