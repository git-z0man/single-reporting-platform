...
The AEV table gained a fourteenth field. Page still returns HTTP 200 at `cra-srp-glossary2`; the footer's version number stayed "1.3" but its date finally moved, for the first time since the 2026-09-10 13:11 UTC content bump.

**New**
- **v26a, Date and time when the Actively Exploited Vulnerability occurred (UTC time)**: inserted between v26 and v27, without renumbering either — AEV field count 12 → 13 (39 → 40 fields overall). Meaning: "Known or estimated date and time when the actively exploited vulnerability occured." (typo verbatim — missing the second "r" in "occurred"). How to complete: "For example, enter the date and time since when the vulnerability exists. If the exact time is unknown, enter the best supported estimate and identify it as estimated in the vulnerability description." Example "2026-08-24 09:15 UTC", Format "Date and time", status Optional (EW24h) / **Required** (72h) / By default copied from previous step, or updated (Final Report). This mirrors the SI table's existing i38 ("Date and time when the incident occurred (UTC time)") — AEV notifications previously had only a "became aware" timestamp (v26); they now also carry an "occurred" timestamp, matching the SI side's long-standing v37/i38 pair. Required at the 72-hour stage, so this is a new mandatory AEV data field, not merely descriptive.

**Watch**
- v26a carries no footnote marker, unlike v26 (footnote [1]) and i37 (footnote [2]) — its "occured" typo is the only defect on it so far.

**Unchanged**
All 18 Common fields, the other 12 AEV fields (v19–v30) and all 9 SI fields (i31–i39), both footnotes; version number "1.3".
...
Page still returns HTTP 200 at `cra-srp-glossary2`, fetched alongside all ten other tracked SRP pages in the same run (one of which — AR Interface Functions, a single invisible `&nbsp;` insertion — changed this time; see `enisa-srp-faq-baseline.md`). The Glossary itself was unaffected: footer still "Version 1.3. Last update: 10 September 2026", field count unchanged (39), all 39 rows re-verified cell by cell against the raw `<table>` structure, including field 10's `cryptoprocessing&nbsp; devices` gap, v27's doubled full stop, v28's empty Example, and v30's missing space in "(CDaC)taking". `retrieved` and `last_check` updated; `last_change` unchanged (still 2026-09-12).

### 2026-09-13 23:15 UTC (vs. 2026-09-12 13:10 UTC)

Page still returns HTTP 200 at `cra-srp-glossary2`, fetched alongside all ten other tracked SRP pages in the same run (nine of which — the FAQ page and all four guidance subpages — gained a widespread bold-emphasis pass this time; see `enisa-srp-faq-baseline.md`). The Glossary itself was unaffected: footer still "Version 1.3. Last update: 10 September 2026", field count unchanged (39), all 39 rows re-verified cell by cell against the raw `<table>` structure, including field 10's `cryptoprocessing&nbsp; devices` gap, v27's doubled full stop, v28's empty Example, and v30's missing space in "(CDaC)taking". `retrieved` updated; `last_check` and `last_change` unchanged (already 2026-09-13 / 2026-09-12 respectively).
...
Page still returns HTTP 200 at `cra-srp-glossary2`, fetched alongside all ten other tracked SRP pages in the same run (none of which changed this time — see `enisa-srp-faq-baseline.md`). Footer unchanged, still "Version 1.3. Last update: 10 September 2026"; field count unchanged (39). One finding, and it corrects this file's own entry from an hour earlier the same day.

**Watch**

- **Field 10 (Product category), Example — the "Fixed" call from 05:12 UTC today does not hold up**: the cell again shows a double-space-like gap before "devices" in the raw markup, now `cryptoprocessing&nbsp; devices` (an `&nbsp;` entity plus a literal space) rather than the two plain spaces originally tracked from 2026-09-10 13:11 UTC. This run cannot tell whether the 05:12 UTC check misread the markup or ENISA edited the cell again in the intervening hour; the row below now reflects the current live text, and the 05:12 UTC "Fixed" entry above should be read as superseded by this one.

**Unchanged**

All other 38 rows, both footnotes, the 42 "By default copied from previous step, or updated" cells, and v28's now-empty Example, all re-verified row by row; version footer and field count.

### 2026-09-12 05:12 UTC (vs. 2026-09-11 10:09 UTC)

Page still returns HTTP 200 at `cra-srp-glossary2`, fetched alongside all ten other tracked SRP pages in the same run (none of which changed this time — see `enisa-srp-faq-baseline.md`). Footer unchanged, still "Version 1.3. Last update: 10 September 2026"; field count unchanged (39, no field added/removed/renumbered). One page-wide terminology change plus two small fixes.

**Changed**

- **72h/Final Report status label expanded page-wide**: every one of the 42 cells across all three tables that read "copied-or-updated" now reads "By default copied from previous step, or updated" instead — the abbreviated status label used since this file's first capture (2026-09-07) has been replaced with the fuller explanation. No status semantics changed (the underlying behaviour — copied from the previous stage by default, editable — is the same); this is a wording expansion, not a new rule.

**Fixed**

- **Field 10 (Product category), Example**: the double-space typo before "devices" ("cryptoprocessing  devices"), tracked since 2026-09-10 13:11 UTC, is gone — now a single space.

**Editorial**

- **v28 (PEC), Example**: the em-dash placeholder ("—") is gone; the cell is now genuinely empty (a non-breaking space only) rather than holding a placeholder character.

**Unchanged**

All field names, Meaning, How-to-complete text (other than field 10's typo fix above), Format, and EW24h statuses; both footnotes; version footer and field count.

### 2026-09-11 10:09 UTC (vs. 2026-09-11 08:09 UTC)

Page still returns HTTP 200 at `cra-srp-glossary2`, fetched alongside all nine other tracked SRP pages in the same run (none of which changed this time — see `enisa-srp-faq-baseline.md`). One real, purely punctuational change on the Glossary itself: field **v23**'s "How to complete" text lost its trailing full stop after "(max. 2000 characters)" — was "...security patches or software updates (max. 2000 characters)."; now ends without the period. No other cell in any of the 39 rows moved (re-verified row by row against the raw `<table>` structure), and the footer is unchanged, still "Version 1.3. Last update: 10 September 2026". `retrieved` updated; `last_check` and `last_change` both updated to 2026-09-11.
...
- Version: 1.3 (moved from 1.2 on 2026-09-10 13:11 UTC; unchanged by the 2026-09-26 field addition)
- Page's own "last update" footer: 25 September 2026 (moved from 10 September 2026 on 2026-09-26, correlating with the new v26a field; the previous move was 09/09/2026 → 10 September 2026 on 2026-09-10 13:11 UTC)
...
- Field count: 40 (was 39 from 2026-09-10 13:11 UTC to 2026-09-17; was 38 before that) — 18 Common, 13 AEV (v19–v30 plus v26a, inserted 2026-09-26), 9 SI (i31–i39)
...
| 8 | Product Type (Default/Important Product with Digital Elements/Critical Product with Digital Elements) *(names expanded 2026-09-10 13:11 UTC; was "(Default/Important/Critical)")* | Both | Regulatory category indicating whether the product is default, important, or critical under the CRA classification framework. | For example, select the CRA regulatory type applicable to the product. Choose Important or Critical only where the product falls within the relevant CRA category (Annexes III and IV); otherwise select Default. | Important Product with Digital Elements *(was "Important" until 2026-09-10 13:11 UTC)* | Select one: Default, Important, Critical | Optional | By default copied from previous step, or updated | By default copied from previous step, or updated |
| 9 | Product class *(lowercase "class" since 2026-09-10 13:11 UTC; was "Product Class")* | Both | For an Important Product with Digital Elements *(wording expanded 2026-09-10 13:11 UTC; was "an important product")*, the applicable Class I or Class II category used for conformity-assessment treatment. | For example, for an important product, select the applicable CRA class. Leave the field empty when the product is not classified as an important product or when no class applies. | Class II | Select one configured Class | Optional | By default copied from previous step, or updated | By default copied from previous step, or updated |
| 10 | Product category *(lowercase "category" since 2026-09-10 13:11 UTC; was "Product Category")* | Both | The applicable CRA product category, normally selected from the relevant category list, as mentioned in the CRA Annexes III or IV. | You may select the category that best matches the affected product under the applicable CRA category list. You may use the product’s core functionality, not only its commercial name. | Choose between: Hardware devices with Security Boxes; Smart meter gateways within smart metering systems and other devices for advanced security purposes, including secure cryptoprocessing&nbsp; devices *(the double-space-like gap before "devices" is back, 2026-09-12 06:11 UTC — this file's own 05:12 UTC entry the same day had called it fixed to a single space; the raw markup now reads `cryptoprocessing&nbsp; devices`, an `&nbsp;` entity followed by a literal space, not two plain spaces as originally tracked from 2026-09-10 13:11 UTC. Whether the 05:12 UTC "Fixed" call was itself mistaken or ENISA re-edited the cell in the interim is not determinable from this run alone; treat the 05:12 UTC "Fixed" entry as superseded rather than reliable.)*; or Smartcards or similar devices, including secure elements. | Select one configured category | Optional | By default copied from previous step, or updated | By default copied from previous step, or updated |
| 11 | End of support indicator | Both | Indicates whether the product with digital elements has reached the end of the support period. *(rewritten 2026-09-10 13:11 UTC — the long-tracked contradictory Meaning, describing a user-interface indicator instead of this field's actual Yes/No content, is gone.)* | You may select one: "Yes" when the product with digital elements has reached the end of the support period or "No" when it doesn’t. *(the "is doesn't" typo fixed to "it doesn't" 2026-09-10 13:11 UTC)* | Yes | Select one: Yes or No | Optional | By default copied from previous step, or updated | By default copied from previous step, or updated |
...
| v23 | Details about the security update/corrective measure available *(new field, since 2026-09-10 13:11 UTC)* | AEV | Information on available corrective measures and security updates. | You may add the available corrective measures and security updates. For example, security patches or software updates (max. 2000 characters) *(trailing full stop dropped 2026-09-11 10:09 UTC)* | Security patches | Detailed structured narrative | Optional | Optional | Required |
...
- 2026-09-25: silent heartbeat, no change anywhere across the eleven tracked pages. `last_check` bumped to 2026-09-25 on `main`; no PR, no check-log entry beyond this line, per the daily-rhythm rule in `CLAUDE.md`.
- 2026-09-26 (06:13 UTC): first check of the day. Page fetched at `cra-srp-glossary2`, HTTP 200, alongside all eleven other tracked SRP pages in the same run — the FAQ page also changed this time, Q28's wording finally catching up to present tense post-go-live (see `enisa-srp-faq-baseline.md`). The Glossary itself changed for the first time since 2026-09-17: a new AEV field, v26a ("Date and time when the Actively Exploited Vulnerability occurred (UTC time)"), inserted between v26 and v27 without renumbering either — field count 39 → 40 — required at the 72-hour stage, and carrying its own "occured" typo. Footer moved "Version 1.3. Last update: 10 September 2026" → "Version 1.3. Last update: 25 September 2026" — version number unchanged, date correlating with the new field for the first time since the stamp's last real move. All other 39 fields re-verified row by row against the raw table structure, all previously tracked defects (field 10's `cryptoprocessing&nbsp; devices` gap, v27's doubled full stop, v30's missing space in "(CDaC)taking") confirmed still present. `retrieved`, `last_check`, and `last_change` all updated to 2026-09-26.
